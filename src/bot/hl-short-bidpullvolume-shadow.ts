...
} from "./hl-short-bidpullvolume-policy";
import {
  HlShortAssetSample,
  HlShortBookSample,
  HlShortMinuteCandle,
...
// Read-only forward observer for the second frozen short candidate.
// Deliberately isolated from the live trading path on every axis the live
// owner validates: separate journal file, separate candidate id, separate
// signal id prefix (hlbpv-), separate policy version/signature. The live
// hl-short-live owner tails only its configured breakdown journal and
// rejects any event whose candidate is not hl_bid_pull_break, so events
// written here are structurally unreachable from order submission.
const POLL_MS = 5_000;
const MINUTE = 60_000;
...
interface PersistedRun {
  signalId: string;
  decisionTs: number;
  features: HlShortBidPullVolumeFeatures;
...
interface HlShortBidPullVolumeShadowStateV1 {
  version: 1;
  symbol: "HYPEUSDT";
...
export interface HlShortBidPullVolumeShadowHealthV1 {
  version: 1;
  symbol: "HYPEUSDT";
  candidate: typeof HL_SHORT_BIDPULLVOLUME_CANDIDATE;
  policyVersion: typeof HL_SHORT_BIDPULLVOLUME_POLICY_VERSION;
  ...
  integrity: HlShortBidPullVolumeShadowStateV1["integrity"];
  counters: ShadowCounters;
}
...
function defaultState(now: number): HlShortBidPullVolumeShadowStateV1 {
  return {
    version: 1,
    symbol: "HYPEUSDT",
    candidate: HL_SHORT_BIDPULLVOLUME_CANDIDATE,
    policyVersion: HL_SHORT_BIDPULLVOLUME_POLICY_VERSION,
    policySignature: HL_SHORT_BIDPULLVOLUME_POLICY_SIGNATURE,
    createdAt: now,
    updatedAt: now,
    lastPollAt: null,
    lastDecisionTs: null,
...
function readState(filePath: string, now: number): HlShortBidPullVolumeShadowStateV1 {
  if (!fs.existsSync(filePath)) return defaultState(now);
  const parsed = JSON.parse(fs.readFileSync(filePath, "utf8")) as Partial<HlShortBidPullVolumeShadowStateV1>;
  if (parsed.version !== 1 || parsed.symbol !== "HYPEUSDT" || parsed.candidate !== HL_SHORT_BIDPULLVOLUME_CANDIDATE) {
    throw new Error("unsupported HYPE HL bid-pull-volume shadow state");
  }
  if (parsed.policyVersion !== HL_SHORT_BIDPULLVOLUME_POLICY_VERSION || parsed.policySignature !== HL_SHORT_BIDPULLVOLUME_POLICY_SIGNATURE) {
    throw new Error("HYPE HL bid-pull-volume shadow policy changed; archive state before starting a new observation cohort");
  }
  if (!parsed.integrity) parsed.integrity = { healthy: true, reason: null, observedAt: null, gapMs: null };
  return parsed as HlShortBidPullVolumeShadowStateV1;
}
...
export class HlShortBidPullVolumeShadow {
  ...
  private state: HlShortBidPullVolumeShadowStateV1;
  ...
  constructor(rootDir: string = process.cwd(), now: number = Date.now()) {
    ...
    this.stateFile = path.join(this.dataDir, `${this.symbol}_hl_short_bidpullvolume_shadow_state.json`);
    this.healthFile = path.join(this.dataDir, `${this.symbol}_hl_short_bidpullvolume_shadow_health.json`);
    this.eventsFile = path.join(this.dataDir, `${this.symbol}_hl_short_bidpullvolume_shadow.jsonl`);
  ...
  private openRun(features: HlShortBidPullVolumeFeatures): void {
    const signalId = `hlbpv-${this.symbol}-${features.decisionTs}`;
  ...
  private evaluateDecision(decisionTs: number): void {
    ...
    if (features.fired) {
      const cooldown = this.state.lastRawSignalTs !== null
        && decisionTs - this.state.lastRawSignalTs < HL_SHORT_BIDPULLVOLUME_POLICY.rawSignalCooldownMs;
      if (cooldown) {
    ...
    console.log(`[hl-short-bpv-shadow] decision=${new Date(decisionTs).toISOString()} ready=${features.ready} fired=${features.fired} outcome=${outcome}`);
  }
  ...
  private recordClose(run: PersistedRun, track: PersistedTrack, close: HlShortShadowClose): void {
    ...
    console.log(`[hl-short-bpv-shadow] close ${run.signalId} ${track.mode} ${close.outcome} pnl=${close.pnlPctAfterFees.toFixed(3)}% stress=${close.pnlPctStressFees.toFixed(3)}%`);
  }
    ...
    for (const run of this.state.runs) {
      for (const track of run.tracks) {
        if (track.status === "pending") {
          ...
          track.position = createHlShortBidPullVolumeShadowPosition(track.mode, track.entryTime, entryCandle.open);
          track.status = "open";
        ...
        for (const candle of pendingCandles) {
          const advanced = advanceHlShortBidPullVolumeShadowPosition(track.position, candle);
  ...
  private health(now: number): HlShortBidPullVolumeShadowHealthV1 {
  ...
  poll(now: number = Date.now(), args: { dryRun?: boolean } = {}): HlShortBidPullVolumeShadowHealthV1 {
    const originalState = args.dryRun ? JSON.parse(JSON.stringify(this.state)) as HlShortBidPullVolumeShadowStateV1 : null;
    ...
    if (latestCandle) {
      const latestClosedBoundary = Math.floor((latestCandle.timestamp + MINUTE) / HL_SHORT_BIDPULLVOLUME_POLICY.decisionIntervalMs)
        * HL_SHORT_BIDPULLVOLUME_POLICY.decisionIntervalMs;
      if (latestClosedBoundary <= now) {
        ...
        if (this.state.lastDecisionTs === null) {
          ...
          if (gap > MAX_CATCHUP_MS) {
            ...
            for (let ts = this.state.lastDecisionTs + HL_SHORT_BIDPULLVOLUME_POLICY.decisionIntervalMs; ts <= latestClosedBoundary; ts += HL_SHORT_BIDPULLVOLUME_POLICY.decisionIntervalMs) decisions.push(ts);
          }
        }
    ...
    if (!this.dryRun) {
      ...
      if (!stateWrite.success) {
        ...
        console.error(`[hl-short-bpv-shadow] state write failed: ${stateWrite.error}`);
        health = this.health(now);
      }
      const healthWrite = atomicWriteJson(this.healthFile, health);
      if (!healthWrite.success) console.error(`[hl-short-bpv-shadow] health write failed: ${healthWrite.error}`);
    }
...
async function main(): Promise<void> {
  ...
  if (symbol !== "HYPEUSDT") throw new Error("hl-short-bidpullvolume shadow is frozen for HYPEUSDT only");
  const shadow = new HlShortBidPullVolumeShadow(rootDir);
  if (once) {
  ...
  console.log(`[hl-short-bpv-shadow] ${symbol} ${HL_SHORT_BIDPULLVOLUME_CANDIDATE} started; shadowOnly=true policy=${HL_SHORT_BIDPULLVOLUME_POLICY_SIGNATURE} poll=${POLL_MS / 1000}s`);
  while (true) {
    try {
      const health = shadow.poll();
      if (health.status !== "healthy") console.log(`[hl-short-bpv-shadow] status=${health.status} reasons=${health.statusReasons.join(",") || "none"}`);
    } catch (err: any) {
      console.error(`[hl-short-bpv-shadow] poll failed: ${err?.message ?? err}`);
    }
...
if (require.main === module) {
  const fatal = installFatalDiagnostics("hype-hl-short-bpv-shadow");
