/** I11 individual OBV/MFI/CMF. Pinned standalone accounting copied unchanged. */
import type { Candle } from "../src/fetch-candles";
import type { ResearchFeature } from "../src/research/indicator-features";
import type { VolumeFlowValues } from "../src/research/volume-flow-features";
...
export interface VolumeFlowRule {
  id:string; family:"obv"|"mfi"|"cmf"|"clock"; side:"long"|"short"; exit:"fixed12h"|"indicator_or12h";
  timeframeMs:number; period:number; threshold:number; mode:"trend"|"into"|"recovery";
}
export type VolumeFlowResult=Omit<StandaloneResult,"rule">&{rule:VolumeFlowRule};
export interface VolumeFlowFeature extends ResearchFeature,VolumeFlowValues {vfPeriod:number;vfFamily:number}
export type VolumeFlowFeatureBar=Omit<FeatureBar,"feature">&{feature:VolumeFlowFeature};
const finite=(x:unknown):x is number=>typeof x==="number"&&Number.isFinite(x);
function validateRule(r:VolumeFlowRule):void {
  const periods=r.family==="mfi"?[7,14,28]:[10,20,40];
  const levels=r.family==="obv"?(r.mode==="trend"?[0,.25,.5,.75]:[.25,.5,.75]):
    r.family==="mfi"?(r.mode==="trend"?[0,.2,.6,.8]:[.6,.8,.9]):
    r.mode==="trend"?[0,.05,.2,.5]:[.05,.2,.5];
  const valid=r.family==="clock"?r.period===14&&r.threshold===0&&r.mode==="trend"&&r.exit==="fixed12h"&&r.timeframeMs===HOUR:
    periods.includes(r.period)&&levels.includes(r.threshold);
  if(!["obv","mfi","cmf","clock"].includes(r.family)||!["long","short"].includes(r.side)||!["fixed12h","indicator_or12h"].includes(r.exit)
    ||!["trend","into","recovery"].includes(r.mode)||![5*MINUTE,15*MINUTE,30*MINUTE,HOUR,4*HOUR].includes(r.timeframeMs)||!valid)
    throw Error("Outside frozen volume-flow grid");
}
function at(r:VolumeFlowRule,b:FeatureBar|undefined):Partial<VolumeFlowFeature>|null {
  if(!b||b.barEnd!==b.candle.timestamp+r.timeframeMs||b.availableAt!==b.barEnd||b.feature.timestamp!==b.candle.timestamp)return null;
  const f=b.feature as Partial<VolumeFlowFeature>;
  return f.vfPeriod===r.period&&f.vfFamily===(r.family==="obv"?1:r.family==="mfi"?2:3)?f:null;
}
export function volumeFlowEntrySignal(r:VolumeFlowRule,prev:FeatureBar|undefined,cur:FeatureBar,_older?:FeatureBar):boolean {
  validateRule(r);if(r.family==="clock")return cur.barEnd%(12*HOUR)===0;
  if(!prev||prev.barEnd!==cur.candle.timestamp)return false;
  const a=at(r,prev)?.vfSigned,b=at(r,cur)?.vfSigned,sign=r.side==="long"?1:-1;
...
export function volumeFlowExitSignal(r:VolumeFlowRule,cur:FeatureBar|undefined):boolean {
  if(r.family==="clock"||r.exit==="fixed12h")return false;
  const v=at(r,cur)?.vfSigned;if(!finite(v))return false;
...
export function runVolumeFlowStandalone(minutes: readonly Readonly<Candle>[], features: readonly FeatureBar[], rule: VolumeFlowRule, o: StandaloneOptions): VolumeFlowResult {
  validateRule(rule);
  ...
  features.forEach((current, i) => {
    ...
    if (rule.family !== "clock" && !at(rule, current)) throw new Error("VolumeFlow parameter provenance mismatch");
  ...
  for (const c of selected) {
    ...
    if (p && !exit) {
      ...
      const neutral = t > p.entryAt && volumeFlowExitSignal(rule, context?.current);
      if (timedOut || neutral) {
    ...
    if (clockTrigger || (rule.family !== "clock" && context && volumeFlowEntrySignal(rule, context.previous, context.current, context.older))) {
      rawSignals++;
