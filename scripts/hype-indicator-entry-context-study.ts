/** Approved C02 and T01 only; frozen C01 sources remain unchanged. Local immutable artifacts; never imports live owners. */
import fs from "fs";
...
import { entryContextGate, runEntryContext as runCombination } from "./indicator-entry-context-policy";
import { calendarAttribution } from "./indicator-calendar-attribution";
...
async function main(){
  ...
  assert(args.length===4&&args[0]==="--study"&&["c02","t01"].includes(args[1])&&args[2]==="--out");
  const card="research-inputs/indicators/"+(args[1]==="c02"?"combinations-c02":"time-gating-t01")+"-2026-09-07.json";
  const spec=read(path.join(root,card)),counts=spec.expectedCounts;
  const out=path.resolve(root,args[3]),relative=path.relative(path.join(root,"backtests"),out);
  assert(relative&&!relative.startsWith("..")&&!path.isAbsolute(relative)&&!fs.existsSync(out),"Require a NEW local backtests directory");
  const n=spec.study==="c02"?32:12,k=spec.study==="c02"?4:6;
  assert.equal(spec.combinations.length,n);assert.equal(spec.anchors.length,n/k);assert.equal(spec.conditions.length,k);
  spec.combinations.forEach((c:any,i:number)=>assert.deepEqual(c,{id:spec.study.toUpperCase()+"-"+String(i+1).padStart(2,"0"),
    anchor:(spec.study==="c02"?"P":"T")+(Math.floor(i/k)+1),condition:spec.study==="c02"?"Q"+(i%k+1):"UTC"+(i%k)}));
  ...
  const originalPins=new Map<string,any>(),archive=new Map<string,any>(),selectedIds=new Set([...spec.anchors.map((r:any)=>r.parentId),"clock_long","clock_short"]);
  ...
  for(const [name,study]of Object.entries(studies) as [string,any][]){
    ...
    for(const pin of [...m.sources,...(m.inputs??[]),...(m.inputRecovery??[]).map((p:any)=>({file:p.effectiveFile,sha256:p.sha256,bytes:p.pinnedBytes}))]){const previous=originalPins.get(pin.file);if(previous)assert.equal(previous.sha256,pin.sha256,pin.file);else originalPins.set(pin.file,pin);}
    ...
    for(const r of summaries){if(archivedRows.has(key(r)))continue;newlyLoaded.add(key(r));archivedRows.set(key(r),r);archivedTrades.set(key(r),[]);archivedMonths.set(key(r),[]);}
    const relevant=newlyLoaded;
  ...
  const sources=[card,"scripts/hype-indicator-entry-context-study.ts","scripts/indicator-entry-context-policy.ts","scripts/indicator-entry-context-features.ts",
    "scripts/indicator-calendar-attribution.ts","scripts/indicator-entry-context-tests.ts","scripts/indicator-entry-context-reference.ts",
    "scripts/indicator-entry-context-results-check.ts",...originalPins.keys()].filter(f=>f!==recoverySource);
  ...
  const features=prepareFeatures(minutes),fields=["id","family","side","exit","timeframeMs","upper","period","threshold","mode","reference","fastPeriod","slowPeriod","signalPeriod","lookbackBars","magnitudePct","multiplier"];
  ...
  assert.equal(parity.length,counts.repeatedCases);console.log("[parity] "+parity.length+" archived controls exact; all-true engines match");
  if(spec.calendarAudit){
    const calendar=results.filter(r=>r.kind!=="clock").flatMap(r=>calendarAttribution(r,cache.get(key(r))!.run));
    json(path.join(out,"calendar-descriptive.json"),calendar);csv(path.join(out,"calendar-descriptive.csv"),calendar);
    console.log("[T01] unfiltered calendar opportunity/occupancy/exposure audit saved BEFORE exclusion variants");
  }
  ...
  for(const c of spec.combinations){
    ...
    const gate=(t:number)=>entryContextGate(c.condition,r.side,anchor.role,t,bRows.get(c.condition)!);
    const po={...options(spec.windows[0],MINUTE),end:prefixEnd};
    const a=runCombination(minutes,os,r.side,po,gate);
    const b=runCombination(prefixTape,prefixFeatures.opportunities(r),r.side,po,t=>entryContextGate(c.condition,r.side,anchor.role,t,prefixFeatures.conditionRows(c.condition)));
    ...
    console.log(`[entry-context] ${c.id} ${c.anchor}+${c.condition}: four cases and prefix passed`);
  }
  ...
  const manifest={version:1,spec,cardFile:card,executionAuthorization:spec.executionAuthorization,node:process.version,
    sources:sourcePins,inputRecovery,seed,minuteRows:minutes.length,rules,featurePrefixChecks,enginePrefixChecks,
    actualPrimaryCases:counts.primaryCases,availabilityLogicalCases:counts.readinessCases,availabilityExecutedCases:readinessRuns,availabilityReusedCases:counts.readinessCases-readinessRuns,
  ...
  json(path.join(out,"validation.json"),{passed:true,combinationDefinitions:n,combinationCases:n*4,repeatedCases:counts.repeatedCases,
    availabilityLogicalCases:counts.readinessCases,exactSavedParentCases:counts.repeatedCases,allTrueEngineCases:counts.allTrueCases,featurePrefixChecks,enginePrefixChecks,
