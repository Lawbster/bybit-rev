/** L15 independent raw-price, target, inventory, monthly and comparison audit. */
import fs from "fs";
...
import { PARENT, validate, POLICIES } from "./age10-gate-policy";
...
async function main() {
  ...
  assert.equal(rows.length, 46); assert.equal(primary.length, 36); assert.equal(rows.filter(x => x.phase === "source60").length, 10);
  ...
  for (const x of rows) {
    ...
    const contexts: R[] = load(`${x.name}-entry-contexts.json`); assert.equal(contexts.length, sizes.length);
    for (let j = 0; j < contexts.length; j++) {
      const c = contexts[j], s = sizes[j];
      assert.equal(c.at, s.at); assert.equal(c.index, s.index); assert.equal(c.nextDepth, s.nextDepth); assert.equal(c.episode, s.episode);
      assert.equal(c.priceDropOk, s.priceDropOk);
      for (const source of Object.values(c.sources) as R[]) if (source) assert(source.availableAt <= c.at && source.sourceAt <= c.at);
      if (c.lastTp) { assert(c.lastTp.at <= c.at); assert(c.lastTp.rsiKnownAt <= c.lastTp.at);
        assert.equal(c.hotUntil, c.lastTp.rsi > cfg.tpCooldown.rsi1hThreshold ? c.lastTp.at + cfg.tpCooldown.cooldownMin * 60000 : 0); }
      assert.equal(c.hotWouldBlock, c.at < c.hotUntil);
      assert.equal(c.deepRawBlocked, c.nextDepth > cfg.deepAddStressGuard.minDepth && !c.priceDropOk &&
        [c.fdByNow, c.fdBnNow, c.fdHlNow].some(v => v !== null && v < cfg.deepAddStressGuard.fundingRateMax));
      assert.equal(c.deepWouldBlock, c.deepRawBlocked && !c.supportReopen);
      if (!x.spec.legs.includes("minus_deep_stress")) assert(!c.deepWouldBlock);
      if (!x.spec.legs.includes("minus_tp_cooldown")) assert(!c.hotWouldBlock);
    }
  ...
  assert.equal(controls, 24); assert.equal(load("control-parity.json").cases.length, controls);
  ...
  const diagnostic = attribution(rows,
    name => fs.readFileSync(path.join(out, `${name}-inventory.jsonl`), "utf8").trim().split("\n").filter(Boolean).map(l => JSON.parse(l)),
    name => load(`${name}-entry-contexts.json`));
  assert.deepEqual(load("entry-attribution.json"), diagnostic);
  await verifyPins(root, [...plan.pins, ...plan.protectedPins, ...state.artifacts]);
  const receipt = { passed: true, economicCases: rows.length, controlsExact: controls, fills, minutes, targetChecks, highChecks, sizeChecks, attributionCases: diagnostic.analyses.length, newTradingDefinitions: 3, liveChanges: 0 };
