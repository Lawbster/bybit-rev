/** SM01 research copy. Only addition: injectable saved S/R context; canonical/live engine unchanged. */
...
export interface CausalRunOptions {
  researchSrContextFactory?: () => Pick<ReplaySrContext, "at">;
...
export function runCausalLongReplay(params: EngineParams, s: Series, opts: CausalRunOptions, liveConfig: BotConfig, initialEquity: number): EngineResult {
  ...
  const sr = opts.researchSrContextFactory ? opts.researchSrContextFactory() : config.srShadow ? new ReplaySrContext(s.candles, config.srShadow) : null;
