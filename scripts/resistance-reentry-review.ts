/** Typechecked review adapter: only adds a non-null annotation to the frozen
 * checker's comparison lookup (TS2532). No economic or verification rule change.
 * Frozen original retained byte-for-byte in job provenance. */
/** SRR01 independent (derived from accepted SRT03 checker); adds full add-eligibility and wait reconstruction. ledger/decision chronology reconstruction. Does not run the
...
import {auditWait,reconstruct,diagnosticSummary} from './resistance-reentry-audit';
...
async function main(){
 ...
 const totals={runs:0,controls:0,minutes:0,fills:0,tpFills:0,targetTransitions:0,selected:0,partialChecks:0,highChecks:0,prefixChecks:0},reviews:R[]=[],traces:R[]=[],waitReviews:R[]=[],ddTraces:R[]=[];
 for(const x of rows){console.log(`[SRR01 verify] ${x.name}`);
  ...
  const probes:R[]=load(x.name,'probes'),activations:R[]=load(x.name,'activations');
  const wa=auditWait(cs,x,events,targets,selections,probes,activations);assert.deepEqual(wa,x.waitAudit);
  assert.equal(wa.blockedChecks,raw.blocked.researchAddVeto??0);
  const reconstructed=reconstruct(cs,x,events,targets,selections,probes);
  if(x.cell==='D')assert.deepEqual(reconstructed,read(`${d.auditDirectory}/${x.name}.json`),'sealed pre-economics audit reproduced');
  waitReviews.push({name:x.name,...wa,diagnostic:diagnosticSummary(reconstructed)});
  ...
  let ptr=0,inv:any[]=[],ep=0,stateEp=-1,phase:'unseen'|'extending'|'released'='unseen',active:R|null=null,held:R|null=null,srCooldown=0,prefix=0,cooldown=0,lastAdd=0,realized=0;
  ...
  for(let i=x.startIdx;i<x.endIdx;i++){
   ...
   while(events[ptr]?.event.fillIndex===i){const z=events[ptr++],e=z.event;
    ...
    inv=z.after;ep=z.episode;lastAdd=z.lastAddTime;active=null;held=null;
   }
 ...
 assert.equal(rows.length,24);assert.equal(totals.controls,8);
 const qualification=d.variants.map((v:string)=>{const failures:string[]=[];
  for(const x of rows.filter(x=>x.waitMode===v)){const q=comparisons.find(r=>r.name===x.name)!.delta,scr=d.screen,review=reviews.find(r=>r.name===x.name)!;
 ...
 const levelReleaseFailures:string[]=[];
 for(const x of rows.filter(x=>x.waitMode==='breakout4h')){
  const w=rows.find(y=>y.cell==='wait4h'&&y.tp===x.tp&&y.window===x.window&&y.lag===x.lag)!;
  const q=exposureDelta(x.metrics,w.metrics);
  if(q.pnlDelta< -1e-7)levelReleaseFailures.push(`${x.name}:net_vs_W`);
  if(!x.lag){if(q.ddReductionPp< -1e-9)levelReleaseFailures.push(`${x.name}:dd_vs_W`);if(q.worstMonthDelta< -250)levelReleaseFailures.push(`${x.name}:monthly_vs_W`);}
 }
 const peerReviews=rows.filter(x=>!x.control).map(x=>({name:x.name,peers:['A','D','wait4h'].map(cell=>{
  const b=rows.find(y=>y.cell===cell&&y.window===x.window&&y.tp===x.tp&&y.lag===(cell==='wait4h'?x.lag:0))!;
  const stored=comparisons.find(q=>q.name===x.name)![cell];assert.deepEqual(stored.delta,exposureDelta(x.metrics,b.metrics));assert.deepEqual(stored.attribution,componentAttribution(x.metrics,b.metrics));
  return{cell,...stored,concentration:cohort(x,b)};
 })}));
 await verifyPins(process.cwd(),pins);atomicJson(`${dir}/review.json`,{qualification,reviews,traces,waitReviews,ddTraces,peerReviews,levelRelease:{passes:!levelReleaseFailures.length,failures:levelReleaseFailures},preaudit:read(`${d.auditDirectory}/summary.json`)});
