/** AG10-E1: exactly two economic variants and three unchanged controls. */
import fs from "fs";
import path from "path";
import assert from "assert/strict";
import { createPlan, executePlan, atomicJson, sha, type Plan } from "./research-workflow";
import { CONTROLS, VARIANTS, LEAD, validate, config, AgeHighController, hourlyWeakness, selectiveBlock, SelectiveContext, qualification, type SelectivePolicy, type R } from "./aggressive10-selective-policy";
...
export async function runStudy(plan: Plan, out: string) {
  ...
  const { PriceContext } = await import("./aggressive10-discriminator-context");
  ...
  console.log("[AG10-E1] Building unchanged closed-bar series; 12 exact controls before variants");
  ...
  const high = trailingHighIndices(s.candles, 2880), context = new SelectiveContext(new PriceContext(s.candles));
  ...
  const run = (base: R, p: SelectivePolicy, phase: string, lag = 0, control?: R) => {
    ...
    console.log(`[AG10-E1 ${results.length + 1}/${d.runs}] ${name}`);
    ...
    const events: ResearchInventoryEvent[] = [], targets: R[] = [], blockedEpisodes = new Set<number>();
    ...
    const r = runCausalLongReplay(params, s, { startIdx, endIdx, recordSnapshots: true, partialClockModel: "transactional",
      researchTpDeferral: age?.decide, researchTpObserver: t => {
        ...
        const gate = observer.entry(add, c), feature = p.restraint && gate.deepWouldBlock ? context.at(add.at, lag) : null;
        const weak = feature ? hourlyWeakness(p.restraint, feature) : false;
        const blocked = selectiveBlock(p.restraint, gate.deepWouldBlock, weak);
        if (blocked) blockedEpisodes.add(add.episode);
        const approvedNotional = blocked ? 0 : add.requestedNotional;
        contexts.push(gate); attempts.push({ ...add, approvedNotional, blocked, weak, feature });
        return approvedNotional;
    ...
    const row = { name, phase, primary: phase !== "source60", policy: p.id, spec: p, model: base.model, window: base.window, tp: base.tp,
      ...
      counts: { attempts: attempts.length, scopeAttempts: contexts.filter(g => g.deepWouldBlock).length,
        blockChecks: attempts.filter(a => a.blocked).length, blockEpisodes: blockedEpisodes.size,
        unknownChecks: attempts.filter(a => a.weak === null).length, targetEpisodes: new Set(age?.observations.filter(o => o.status === "extended").map(o => o.episode)).size,
...
async function main() {
  ...
  if (cmd === "plan") {
    ...
    const sources = [...inherited, arg, "scripts/hype-aggressive10-selective-study.ts", "scripts/aggressive10-selective-policy.ts", "scripts/aggressive10-selective-sources.ts",
      "scripts/aggressive10-selective-tests.ts", "scripts/aggressive10-selective-verify.ts", "scripts/aggressive10-discriminator-context.ts", "scripts/aggressive10-discriminator-study.ts"];
    ...
    console.log(JSON.stringify({ key: plan.key, definitionId: plan.definitionId, next: `npx ts-node scripts/hype-aggressive10-selective-study.ts run ${plan.key}` }));
