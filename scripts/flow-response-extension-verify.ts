...
async function main() {
  ...
  assert.equal(controls, 6); await verifyPins(root, pins);
  ...
  const receipt = { passed: true, cases: 14, controlsExact: controls, fills, minutes, addChecks: checks, featureChecks, targetChecks, highChecks,
    uniqueFeatureWindows: source.cache.size, newDefinitions: 0, liveChanges: 0, checkerSha256: await fileHash(__filename) };
