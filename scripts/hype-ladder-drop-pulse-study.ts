...
import type { DropSpec, DropVariant } from "./ladder-drop-pulse-policy";
...
async function main() {
  const root = path.resolve(__dirname, ".."), definition = "research-inputs/sr-pulse-encounters/ladder-drop-pulse-2026-09-05.json";
  const read = (file: string) => JSON.parse(fs.readFileSync(path.resolve(root, file), "utf8"));
  const spec: DropSpec = read(definition), cfg: BotConfig = read("bot-config.json");
  ...
  const previousStudy = read(`${spec.priorExposureDir}/manifest.json`);
  ...
  console.log("[manifest] prior exposure engine/sources unchanged; hash preserved inputs and frozen genuine-drop rules");
  const inputs: any[] = [];
  ...
  const sourceFiles = [...new Set<string>([...previousStudy.sources.map((s: any) => s.file), definition, "scripts/hype-ladder-drop-pulse-study.ts",
    "scripts/ladder-drop-pulse-policy.ts", "scripts/ladder-exposure-metrics.ts", "scripts/ladder-drop-pulse-tests.ts", "package-lock.json", "tsconfig.json"])];
  const sources = sourceFiles.map(file => ({ file, sha256: hash(fs.readFileSync(path.join(root, file))) }));
  const out = path.resolve(root, process.env.DROP_PULSE_OUT ?? `backtests/hype/${spec.id}`), rel = path.relative(path.join(root, "backtests"), out);
  ...
  json("manifest.json", { at: new Date().toISOString(), node: process.version, spec, inputs, sources,
    ...
    previousStudyManifestSha256: hash(fs.readFileSync(path.join(root, spec.priorExposureDir, "manifest.json"))),
  ...
  const { dropVariants, dropVeto } = await import("./ladder-drop-pulse-policy");
  ...
  const variants = dropVariants(spec); assert.equal(variants.length, spec.variantCount);
  ...
  for (const prior of [...expected].sort((a: any, b: any) => Number(!a.id.startsWith("hl_window")) - Number(!b.id.startsWith("hl_window")))) {
    ...
    const run = (variant: DropVariant | null) => {
      ...
      const r = runCausalLongReplay({ ...params, id: variant ? label : prior.id }, s, { ...opts, researchAddVeto: d => {
        ...
        const answer = dropVeto(variant, d, spec);
        ...
        if (answer.veto) {
          assert(d.priceDropOk && d.nextDepth >= variant.depth, "Genuine-drop scope violated");
      ...
      const data = { model: prior.id, variant: variant?.id ?? "baseline", family: variant?.family ?? "baseline", depth: variant?.depth ?? null,
        digest, opportunities, timerOpportunities, vetoMinutes, vetoEpisodes: episodes.size, unknown, unknownReasons, releaseCounts, maxKnownAt, metrics };
  ...
  json("validation.json", { allFourBaselineDigestsMatched: true, causalFillsAndKnownResistance: true, genuineDropScopeOnly: true, sourceAndInputHashesUnchanged: true,
