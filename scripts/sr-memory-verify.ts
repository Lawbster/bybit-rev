...
async function main(){
  ...
  for(const x of rows){
    ...
    targetChecks+=auditAgeHighTargets(cs,events as any,targets,load(x.name,'tp-observations'),{...x,control:!x.spec.ageHours,policy:{id:'baseline',family:'age'},sensitivity:{sourceLagMs:0,releaseDelayMs:0}},{},c,{},t=>arms.push(t)).checks;
    ...
    for(const e of events.filter(e=>e.event.reason==='sr_partial')){
      ...
      assert(ctx.coverage.healthy);assert(at>=cooldown);cooldown=e.event.fillAt+c.srPartialExitAction!.cooldownMin*60000;
    ...
    summaries.push({name:x.name,window:x.window,tp:x.tp,days:x.days,policy:x.policy,control:x.control,metrics:x.metrics,monthly:x.accounting.monthly,delta,changedPartialEpisodes:changed.size,
      changedPartialFills:srRows.filter(e=>!signatures.has(`${e.at}|${e.qty}`)).length,unhealthyMinutes:x.blocked.srContext});zoneEvidence.push({name:x.name,partials:srRows});
  }
  ...
  const receipt={passed:true,runs:24,exactControls:controls,fills,targetChecks,highChecks,prefixChecks,reviewSha:sha(fs.readFileSync(`${dir}/review.json`)),liveChanges:0};atomicJson(`${dir}/verification.json`,receipt);console.log(JSON.stringify(receipt));
}
