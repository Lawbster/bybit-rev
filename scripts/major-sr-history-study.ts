...
import {levelTable,renderMap,iso} from './major-sr-history-render';
const CARD='research-inputs/major-sr-history-map01-2026-09-17.json';
...
async function worker(plan:Plan,out:string){
 const d=plan.card.definition;assert.equal(d.runs,0);assert(read(`${d.archive}/verification.json`).passed);
 ...
 assert.deepEqual(o.events,read(`${d.archive}/output/240m-events.json`),'Historical event parity failed');
 assert.deepEqual(o.snapshot(),read(`${d.archive}/output/240m-final.json`),'Final map parity failed');
 ...
 write('summary.json',{key:plan.key,symbol:d.symbol,sourceStart:iso(map.sourceStart),sourceEnd:iso(map.sourceEnd),cutoff:d.cutoff,
   minuteBars:cs.length,complete4hBars:bars.length,missing4hBars:missingBars.map(iso),
...
async function main(){const [cmd,arg,time]=process.argv.slice(2);
 if(cmd==='plan'){
   const card=read(CARD);const sources=['scripts/major-sr-history-study.ts','scripts/major-sr-history-map.ts','scripts/major-sr-history-render.ts',
     'scripts/major-sr-history-tests.ts','scripts/major-sr-history-verify.ts','scripts/macro-sr-observer.ts','scripts/hype-freerun-canonical-replay.ts',
     'scripts/replay-candle-repair.ts','scripts/replay-causality.ts','scripts/research-workflow.ts','package-lock.json','docs/research/major-sr-history-map01.md',`${card.definition.archive}/plan.json`];
 ...
 else throw Error('Usage: major-sr-history-study.ts plan | run KEY | query KEY UTC_ISO');
}
