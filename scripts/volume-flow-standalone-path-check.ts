/** Read-only I11 report supplement. No new signals, exits or strategy runs. */
...
async function main(): Promise<void> {
  ...
  const out = path.resolve(root, process.argv[2] ?? "backtests/hype/hype-volume-flow-standalone-2026-09-07");
  ...
  const shortlist=read("shortlist.json"), ranking=read("ranking.json");
  const eligibleShorts=["obv","mfi","cmf"].flatMap(f=>shortlist.eligibleIds.filter((id:string)=>
    ranking.find((r:any)=>r.id===id)?.family===f&&ranking.find((r:any)=>r.id===id)?.side==="short").slice(0,5));
  const ids = ["clock_long", "clock_short", ...read("shortlist.json").topFiveIds, ...read("shortlist.json").byFamily.flatMap((r:any)=>r.topFiveIds), ...read("ranking.json").slice(0,5).map((r:any)=>r.id),...eligibleShorts];
  ids.push(...ids.filter(id=>id.endsWith("indicator_or12h")).map(id=>id.replace("indicator_or12h","fixed12h")));
  const trades: any[] = [];
