/** LV03 independent minute-path checker. No producer or replay import. */
...
export function auditCap(cs: readonly Candle[], signals: readonly Row[], row: Row, run: Row, c: Row) {
  ...
  for (const s of signals) {
    ...
    const value = (p: number) => cash + qty * (p - price) - qty * p * c.fee;
    ...
    for (let t = entered; t < Math.min(timeout, o.end); t += M) {
      ...
      if (!timed && (b.low <= stop! || b.high >= target!)) {
        const gapSl = b.open <= stop!, gapTp = b.open >= target!;
        both = !gapSl && !gapTp && b.low <= stop! && b.high >= target!; if (both) ambiguities++;
        reason = gapSl ? 'stop' : gapTp ? 'target' : b.low <= stop! && (!both || !row.targetFirst) ? 'stop' : 'target';
        exitAt = t; exit = gapSl || gapTp ? b.open : reason === 'stop' ? stop : target;
        adverse = value(gapSl || gapTp ? b.open : reason === 'stop' || both ? stop! : b.low); break;
      }
      observe(t, value(b.close), value(b.low));
    }
    if (exitAt < o.end) {
      exit ??= bar(exitAt).open; const gross = qty * (exit - price), exitFee = qty * exit * c.fee, net = gross - entryFee - exitFee;
      ...
      const expected = { id: s.id, signalAt: s.at, entryAt: entered, exitAt, entryPrice: price, exitPrice: exit, side: 1,
        qty, pricePnl: gross, fees: entryFee + exitFee, net, reason, target, stop, r: timed ? null : net / (qty * (price - stop!)), touchedBoth: both, evidence: null };
      ...
      open = { id: s.id, signalAt: s.at, entryAt: entered, entryPrice: price, qty, side: 1, markedPrice,
        net: qty * (markedPrice - price) - entryFee - qty * markedPrice * c.fee };
