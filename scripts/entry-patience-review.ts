...
async function main(){const key=process.argv[2];assert(/^[a-f0-9]{64}$/.test(key));const dir=`backtests/research-workflow/${key}`,out=`${dir}/output`;
  ...
  const originalAudit=fs.readFileSync('scripts/age-high-refinement-audit.ts','utf8').replace(/\r\n/g,'\n');
  const boundary='if (!x.control) for (const k of Object.keys(counts))';assert.equal(originalAudit.split(boundary).length,2);
  assert.equal(fs.readFileSync('scripts/entry-patience-target-audit.ts','utf8').replace(/\r\n/g,'\n').trimEnd(),
    originalAudit.replace(boundary,'if (!x.control && x.audit) for (const k of Object.keys(counts))').trimEnd());
  ...
  for(const x of read(`${out}/ladder-results.json`)){
    ...
    if(x.spec.days)highChecks+=auditAgeHighExits(cs,events as any,{...x,lag:0},read(`${out}/${x.name}-high-reductions.json`),hs);
  ...
  assert.equal(read(`${out}/standalone-results.json`).filter((x:R)=>x.control).length,12);
  assert.equal(read(`${out}/ladder-results.json`).filter((x:R)=>x.control).length,8);
  ...
  atomicJson(`${dir}/verification.json`,{passed:true,standaloneCases:60,ladderCases:56,fills,minutes,windowChecks,targets,highChecks,ladderAccounting:verified,
    exactControlCases:20,liveChanges:0,reviewerSha256:sha(fs.readFileSync(__filename,"utf8")),targetAuditSha256:sha(fs.readFileSync('scripts/entry-patience-target-audit.ts','utf8')),
    checkerDelta:"Explicit zero source lag and optional unpersisted diagnostic counters; every TP decision/observation still checked. Economic inputs/worker unchanged"});console.log(JSON.stringify({passed:true,fills,minutes,windowChecks,targets,highChecks}));
}
