/** Frozen MACD-only standalone research. New local artifacts; no live/config writes. */
import fs from "fs";
...
import { runMacdStandalone, macdEntrySignal, type MacdRule, type MacdResult } from "./macd-standalone-engine";
import { computeMacdValues, MACD_FORMULA_VERSION } from "../src/research/macd-features";
...
async function main() {
  ...
  assert.equal(spec.holdMs, 12 * HOUR); assert.equal(spec.strategyDefinitionCount, 300); assert.equal(spec.symbol, "HYPEUSDT");
  ...
  const sourceFiles = [definition, "scripts/macd-standalone-engine.ts", "scripts/hype-macd-standalone-study.ts",
    "src/research/macd-features.ts", "scripts/macd-standalone-tests.ts", "scripts/macd-standalone-results-check.ts", "scripts/macd-standalone-path-check.ts",
  ...
  const ruleKey = (r: MacdRule) => featureKey(r.timeframeMs,[r.fastPeriod,r.slowPeriod,r.signalPeriod]);
  let prefixChecks = 0;
  for (const tf of spec.timeframesMs as number[]) {
    ...
    for (const p of spec.presets as number[][]) {
      const values=computeMacdValues(closed,tf,p[0],p[1],p[2]);
      ...
      for(const i of [800,Math.floor(rows.length/2),rows.length-3]) {
        ...
        assert.deepEqual(computeMacdValues(prefix.bars.map(b=>b.candle),tf,p[0],p[1],p[2]).at(-1),values[i]);
        assert(bars.windowAt(at,i+1).ready); prefixChecks++;
      }
    }
    console.log(`[features] ${tf/MINUTE}m ${oldRows.length} bars; three MACD presets / nine prefixes passed`);
  }
  const rules: MacdRule[]=[];
  for(const timeframeMs of spec.timeframesMs) for(const [fastPeriod,slowPeriod,signalPeriod] of spec.presets)
    for(const mode of spec.entryModes) for(const side of spec.sides) for(const exit of spec.exits)
      rules.push({id:`macd_${timeframeMs/MINUTE}m_${fastPeriod}_${slowPeriod}_${signalPeriod}_${mode}_${side}_${exit}`,
        family:"macd",timeframeMs,fastPeriod,slowPeriod,signalPeriod,mode,side,exit});
  const controls: MacdRule[]=spec.sides.map((side:"long"|"short")=>({id:`clock_${side}`,family:"clock",side,
    exit:"fixed12h",timeframeMs:HOUR,fastPeriod:12,slowPeriod:26,signalPeriod:9,mode:"signal_cross"}));
  assert.equal(rules.length,300); assert.equal(new Set(rules.map(r=>r.id)).size,300);
  ...
  const cache = new Map<string, MacdResult>(), parity: any[] = [];
  for (const w of spec.windows) for (const delayMs of spec.executionDelaysMs) {
    for (const r of controls) {
      ...
      const fresh=runMacdStandalone(minutes,baseFeatures.get(HOUR)!,r,options(w,delayMs));
  ...
  json(path.join(out, "manifest.json"), { spec, node: process.version, sources, inputs, minuteRows: minutes.length, seed,
    formulaVersion: RESEARCH_FEATURE_FORMULA_VERSION + "|" + MACD_FORMULA_VERSION, timingVersion: CLOSED_BAR_TIMING_VERSION,
  ...
  for (const w of spec.windows) for (const delayMs of spec.executionDelaysMs) for (const rule of [...controls, ...rules]) {
    ...
    const r = cache.get(key(w.id, delayMs, rule.id)) ?? runMacdStandalone(minutes, rows, rule, options(w, delayMs));
    ...
    if (rule.family !== "clock" && w.id === "full" && delayMs === 0 && r.trades.length) {
      ...
      assert(i>0); assert(macdEntrySignal(rule,rows[i-1],rows[i],rows[i-2]));
      const f=(j:number)=>{const x=rows[j].feature as any;return {macdFastEma:x.macdFastEma,macdSlowEma:x.macdSlowEma,macdLine:x.macdLine,macdSignal:x.macdSignal,macdHist:x.macdHist};};
  ...
  assert.equal(results.length, 1208);
  ...
  for (const rule of rules) {
    ...
    assert.deepEqual(runMacdStandalone(minutes, rows, rule, opts), runMacdStandalone(
  ...
  json(path.join(out, "validation.json"), { missingMinutes: 0, overlapParityCases: parity.length, exactSavedOverlap: true,
    ...
    strategyDefinitions: rules.length, newDefinitions: 300, repeatedDefinitions: 0, controls: 2, strategyCases: 1200, controlCases: 8,
