...
import { ReplayVariant, RowFilter, ReplayConfig, cellId, parseRowFilter, rowMatches, buildActions } from '../src/strategies/setup-actions';
export { ReplayVariant, RowFilter, ReplayConfig, cellId, parseRowFilter, rowMatches, buildActions } from '../src/strategies/setup-actions';
...
export async function runReplay(root: string, cfg: ReplayConfig): Promise<{ key: string; dir: string; reused: boolean; report: string }> {
  ...
  const pins: Row[] = [];
  for (const f of ['scripts/setup-replay.ts', 'src/strategies/setup-actions.ts', 'src/strategies/setup-context.ts', 'scripts/poc-indicator-bias-engine.ts', 'scripts/structural-replay-audit.ts', `backtests/setup-scans/${cfg.scanKey}/events.jsonl`, `backtests/setup-scans/${cfg.scanKey}/complete.json`]) pins.push({ file: f, sha256: await fileHash(path.join(root, f)) });
