/** L14: 24 frozen follow-ups, exact controls, full occupancy and source-delay checks. */
import fs from "fs";
import path from "path";
import assert from "assert/strict";
import { createPlan, executePlan, atomicJson, inside, sha, verifyPins, fileHash, type Plan } from "./research-workflow";
import { CONTROLS, VARIANTS, PARENT, validate, config, size, AgeHighController, type Policy, type R } from "./age-high-refinement-policy";
...
export async function runStudy(root: string, plan: Plan, out: string) {
  ...
  console.log("[L14] Building unchanged canonical B17 series; 24 frozen policies, no live changes");
  ...
  for (const days of [1, 2, 3, 5, 6]) { const h = trailingHighIndices(s.candles, days * 1440); highs.set(days, h); fs.writeFileSync(path.join(out, `high-${days}d.i32`), Buffer.from(h.buffer)); }
  ...
  const run = (base: R, p: Policy, phase: string, lag = 0, control?: R) => {
    ...
    console.log(`[L14 ${results.length + 1}/${d.runs}] ${name}`);
    ...
    const r = runCausalLongReplay(params, s, { startIdx, endIdx, recordSnapshots: true, partialClockModel: "transactional",
      ...
      }, researchAddSize: d => { const n = size(p, d); sizes.push({ ...d, approvedNotional: n }); if (n !== d.requestedNotional) sizeEpisodes.add(d.episode); return n; },
      researchReduction: hi?.reduce, researchInventoryObserver: e => { hi?.observe(e); events.push(e); }
    }, c, d.initialEquity);
  ...
  for (const p of CONTROLS) for (const base of bases) {
    const old = accepted.find(x => x.policy === p.id && x.model === base.model)
      ?? (base.window === "published_window" ? components.find(x => x.policy === p.id && x.model === base.model) : null);
    if (!old) assert(["minus_sr_partial", "minus_tp_cooldown"].includes(p.id) && base.window === "hl_extended");
    run(base, p, old ? "exact_control" : "refreshed_single", 0, old ?? undefined);
  }
  assert.equal(parity.length, 20); json("control-parity.json", { passed: true, cases: parity });
  for (const p of VARIANTS) for (const base of bases) run(base, p, "primary");
  for (const p of [CONTROLS.find(p => p.id === PARENT)!, ...VARIANTS]) for (const base of bases.filter(x => x.window === "hl_extended")) run(base, p, "source60", 60000);
  assert.equal(results.length, d.runs); const primary = results.filter(x => x.primary); assert.equal(primary.length, 120);
  ...
  json("validation.json", { passed: true, economicCases: results.length, primaryCases: primary.length, controlsExact: parity.length, newTradingDefinitions: 24, liveChanges: 0, economicQualification: "independent_check_required" });
}
async function main() {
  ...
  if (cmd === "plan") {
    ...
    const sources = [...inherited, arg, "scripts/hype-age-high-refinement-study.ts", "scripts/age-high-refinement-policy.ts", "scripts/age-high-refinement-audit.ts", "scripts/age-high-refinement-tests.ts", "scripts/age-high-refinement-verify.ts",
    ...
    console.log(JSON.stringify({ key: p.key, definitionId: p.definitionId, next: `npx ts-node scripts/hype-age-high-refinement-study.ts run ${p.key}` }, null, 2));
