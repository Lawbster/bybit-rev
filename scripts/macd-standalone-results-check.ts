...
// Independent array-based EMA calculation from directly aggregated minute closes.
// No feature/strategy/closed-bar engine import. Arithmetic order is canonical;
// tests additionally compare against a weighted-sum formula and external library.
function referenceMacd(candles:any[],tf:number,p:number[]):Map<number,any> {
  const step=tf/M,cs:number[]=[],starts:number[]=[];
  for(let i=0;i+step<=candles.length;i+=step){cs.push(candles[i+step-1].close);starts.push(candles[i].ts);}
  const ema=(xs:number[],period:number):Array<number|null>=>{
    const out:Array<number|null>=Array(xs.length).fill(null);
    if(xs.length<period)return out;
    let first=0; for(let i=0;i<period;i++)first+=xs[i]; out[period-1]=first/period;
    for(let i=period;i<xs.length;i++)out[i]=out[i-1]!+2/(period+1)*(xs[i]-out[i-1]!);
    return out;
  };
  const fast=ema(cs,p[0]),slow=ema(cs,p[1]);
  const lines=cs.slice(p[1]-1).map((_,i)=>fast[i+p[1]-1]!-slow[i+p[1]-1]!);
  const signals=ema(lines,p[2]);
  return new Map(starts.map((ts,i)=>{const j=i-p[1]+1,line=j<0?null:lines[j],sig=j<0?null:signals[j];
    return [ts,{macdFastEma:fast[i],macdSlowEma:slow[i],macdLine:line,macdSignal:sig,macdHist:line===null||sig===null?null:line-sig}];}));
}
function crossed(r:any,a:any,b:any,z?:any):boolean {
  if(!a||!b)return false;
  const side=r.side==="long"?1:-1,field=r.mode==="zero_cross"?"macdLine":"macdHist";
  if(a[field]==null||b[field]==null)return false;
  if(r.mode==="hist_turn")return z?.macdHist!=null&&side*(a.macdHist-z.macdHist)<=0&&side*(b.macdHist-a.macdHist)>0&&side*b.macdHist<0;
  if(!(side*a[field]<=0&&side*b[field]>0))return false;
  if(r.mode==="signal_trend")return b.macdLine!=null&&side*b.macdLine>0;
  if(r.mode==="signal_counter")return b.macdLine!=null&&side*b.macdLine<0;
  return true;
}
function zeroExit(r:any,b:any):boolean {
  const v=b?.[r.mode==="zero_cross"?"macdLine":"macdHist"];
  if(v==null)return false;const signed=(r.side==="long"?1:-1)*v;
  return r.mode==="hist_turn"?signed>=0:signed<=0;
}

async function main(): Promise<void> {
  assert.equal(process.argv.length, 3, "Usage: npx ts-node scripts/macd-standalone-results-check.ts ARTIFACT_DIRECTORY");
  ...
  const definition = "research-inputs/indicators/macd-standalone-2026-09-06.json";
  ...
  assert.deepEqual(spec.presets,[[6,13,5],[12,26,9],[24,52,18]]);
  assert.deepEqual(spec.entryModes,["signal_cross","signal_trend","signal_counter","zero_cross","hist_turn"]);
  ...
  for(const timeframeMs of spec.timeframesMs)for(const [fastPeriod,slowPeriod,signalPeriod] of spec.presets)
    for(const mode of spec.entryModes)for(const side of spec.sides)for(const exit of spec.exits)
      expectedRules.push({family:"macd",side,exit,timeframeMs,fastPeriod,slowPeriod,signalPeriod,mode,
        id:`macd_${timeframeMs/M}m_${fastPeriod}_${slowPeriod}_${signalPeriod}_${mode}_${side}_${exit}`});
  expectedRules.push(...spec.sides.map((side:string)=>({family:"clock",side,exit:"fixed12h",timeframeMs:H,
    fastPeriod:12,slowPeriod:26,signalPeriod:9,mode:"signal_cross",id:`clock_${side}`})));
  ...
  assert.equal(expected.size, 1208); assert.equal(summaries.length, expected.size);
  ...
  const referenceKey=(r:any)=>`${r.timeframeMs}:${r.fastPeriod}_${r.slowPeriod}_${r.signalPeriod}`;
  for(const tf of spec.timeframesMs)for(const p of spec.presets){
    const values=referenceMacd(candles,tf,p);referenceByTf.set(`${tf}:${p.join("_")}`,values);
  ...
  for (const [key, r] of byCase) {
    ...
    const firstExit = (entered: number): Exit | null => {
      ...
      if (r.exit === "indicator_or12h") {
        ...
        // Only subsequent closed boundaries; do not reuse entry's MACD value.
    ...
    const validateEntry = (trade: any, label: string) => {
      ...
      if (r.family !== "clock") {
        ...
        assert.ok(crossed(r, references.get(signal - 2 * r.timeframeMs), references.get(signal - r.timeframeMs), references.get(signal - 3*r.timeframeMs)), `${label} independently reconstructed MACD crossing`);
      }
    ...
    for (const [i, t] of rows.entries()) {
      ...
      if (t.entryFeature !== null) {
        ...
        if(r.family!=="clock") {
          ...
          assert.equal(t.entryFeature.macdFastPeriod,r.fastPeriod);
          assert.equal(t.entryFeature.macdSlowPeriod,r.slowPeriod);
          assert.equal(t.entryFeature.macdSignalPeriod,r.signalPeriod);
        }
  ...
  assert.equal(ranking.length, 300);
  ...
  const report = { version: 1, verifiedAt: new Date().toISOString(), passed: true,
    ...
    checks: ["independent MACD formulas for all five timeframes / three EMA presets; signal/sign/zero/slope crossings",
    ...
    limitations: ["No strategy engine is imported; independent array-based MACD formulas/crossing enumeration and ledger accounting check this fixed-timeout/neutral-exit study only.",
      "No untouched holdout or MACD combination tests; shortlist is descriptive, not live approval.",
