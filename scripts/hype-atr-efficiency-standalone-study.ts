/** Frozen AtrEfficiency-only standalone research. New local artifacts; no live/config writes. */
import fs from "fs";
...
import { runAtrEfficiencyStandalone, atrEfficiencyEntrySignal, type AtrEfficiencyRule, type AtrEfficiencyResult } from "./atr-efficiency-standalone-engine";
import { computeAtrEfficiencyValues, ATR_EFFICIENCY_FORMULA_VERSION } from "../src/research/atr-efficiency-features";
...
async function main() {
  ...
  assert.equal(spec.holdMs, 12 * HOUR); assert.equal(spec.strategyDefinitionCount, 720); assert.equal(spec.symbol, "HYPEUSDT");
  ...
  const sourceFiles = [definition, "scripts/atr-efficiency-standalone-engine.ts", "scripts/hype-atr-efficiency-standalone-study.ts",
    "src/research/atr-efficiency-features.ts", "scripts/atr-efficiency-standalone-tests.ts", "scripts/atr-efficiency-standalone-results-check.ts", "scripts/atr-efficiency-standalone-path-check.ts",
  ...
  const ruleKey = (r:AtrEfficiencyRule) => featureKey(r.timeframeMs,r.family,r.period);
  let prefixChecks = 0;
  for (const tf of spec.timeframesMs as number[]) {
    ...
    for (const p of spec.parameters) {
      const values=computeAtrEfficiencyValues(closed,tf,p.family,p.period);
      const rows=bars.bars.map((b,i)=>({...b,feature:{...base[i],...values[i],aePeriod:p.period,aeFamily:p.family==="atr_move"?1:2}}));
      features.set(featureKey(tf,p.family,p.period),rows);
      if(p.family==="atr_move")values.forEach((v,i)=>assert.equal(v.aeAtr,base[i].atr14));
      for(const i of [800,Math.floor(rows.length/2),rows.length-3]){
        ...
        assert.deepEqual(computeAtrEfficiencyValues(prefix.bars.map(b=>b.candle),tf,p.family,p.period).at(-1),values[i]);
        assert(bars.windowAt(at,i+1).ready);prefixChecks++;
      }
    }
    console.log(`[features] ${tf/MINUTE}m ${oldRows.length} bars; four parameter sets /12 prefixes; ATR14 shared exact`);
  }
  const rules:AtrEfficiencyRule[]=[];
  ...
  const controls:AtrEfficiencyRule[]=spec.sides.map((side:"long"|"short")=>({id:`clock_${side}`,family:"clock",side,
    exit:"fixed12h",timeframeMs:HOUR,period:14,threshold:0,mode:"trend"}));
  assert.equal(rules.length,720);assert.equal(new Set(rules.map(r=>r.id)).size,720);
  ...
  const cache = new Map<string, AtrEfficiencyResult>(), parity: any[] = [];
  for (const w of spec.windows) for (const delayMs of spec.executionDelaysMs) {
    for (const r of controls) {
      ...
      const fresh=runAtrEfficiencyStandalone(minutes,baseFeatures.get(HOUR)!,r,options(w,delayMs));
  ...
  json(path.join(out, "manifest.json"), { spec, node: process.version, sources, inputs, minuteRows: minutes.length, seed,
    formulaVersion: RESEARCH_FEATURE_FORMULA_VERSION + "|" + ATR_EFFICIENCY_FORMULA_VERSION, timingVersion: CLOSED_BAR_TIMING_VERSION,
  ...
  for (const w of spec.windows) for (const delayMs of spec.executionDelaysMs) for (const rule of [...controls, ...rules]) {
    ...
    const r = cache.get(key(w.id, delayMs, rule.id)) ?? runAtrEfficiencyStandalone(minutes, rows, rule, options(w, delayMs));
    ...
    if (rule.family !== "clock" && w.id === "full" && delayMs === 0 && r.trades.length) {
      ...
      assert(i>0); assert(atrEfficiencyEntrySignal(rule,rows[i-1],rows[i],rows[i-2]));
      const f=(j:number)=>{const x=rows[j].feature as any;return {aeAtr:x.aeAtr,aePriorAtr:x.aePriorAtr,aeChange:x.aeChange,aeTravel:x.aeTravel,aeUnsigned:x.aeUnsigned,aeSigned:x.aeSigned};};
  ...
  assert.equal(results.length, 2888);
  ...
  json(path.join(out, "shortlist.json"), { interpretation: "Descriptive positive/cost/sample subset; retain monthly/clock failures, no live approval.",
    ...
    byFamily:["atr_move","efficiency"].map(family=>({family,eligibleIds:eligible.filter(x=>x.family===family).map(x=>x.id),
  ...
  for (const rule of rules) {
    ...
    assert.deepEqual(runAtrEfficiencyStandalone(minutes, rows, rule, opts), runAtrEfficiencyStandalone(
  ...
  json(path.join(out, "validation.json"), { missingMinutes: 0, overlapParityCases: parity.length, exactSavedOverlap: true,
    actualDataFeaturePrefixChecks: prefixChecks, atr14SharedMathExact: true, actualDataEnginePrefixChecks: enginePrefixChecks,
    strategyDefinitions: rules.length, newDefinitions: 720, repeatedDefinitions: 0, controls: 2, strategyCases: 2880, controlCases: 8,
