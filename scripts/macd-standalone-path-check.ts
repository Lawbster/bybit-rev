/** Read-only I06 report supplement. No new signals, exits or strategy runs. */
...
async function main(): Promise<void> {
  ...
  const out = path.resolve(root, process.argv[2] ?? "backtests/hype/hype-macd-standalone-2026-09-06");
  ...
  const ids = ["clock_long", "clock_short", ...read("shortlist.json").topFiveIds];
  const trades: any[] = [];
