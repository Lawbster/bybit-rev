/** Read-only I09 report supplement. No new signals, exits or strategy runs. */
...
async function main(): Promise<void> {
  ...
  const out = path.resolve(root, process.argv[2] ?? "backtests/hype/hype-atr-efficiency-standalone-2026-09-06");
  ...
  const ids = ["clock_long", "clock_short", ...read("shortlist.json").topFiveIds, ...read("shortlist.json").byFamily.flatMap((r:any)=>r.topFiveIds), ...read("ranking.json").slice(0,5).map((r:any)=>r.id)];
  const trades: any[] = [];
