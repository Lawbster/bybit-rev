...
export function auditFlowLedger(cs: Candle[], events: ResearchInventoryEvent[], attempts: R[], metrics: ExposureMetrics, start: number, end: number) {
  ...
  function fill(x: ResearchInventoryEvent) {
    ...
    if (e.fillAt === c.ts) { assert.equal(e.fillIndex, e.decisionIndex + 1); near(e.price!, c.open); }
  ...
  for (const a of attempts) {
    if (a.approvedNotional === 0) assert(!opens.has(a.index));
