...
export type ExecutionEvent = { exitFeeRate?: number; executionMode?: string; kind: string; decisionAt: number; fillAt: number | null; decisionIndex: number; fillIndex: number | null; price: number | null; qty: number; reason: string };
...
export function runCausalLongReplay(params: EngineParams, s: Series, opts: CausalRunOptions, liveConfig: BotConfig, initialEquity: number): EngineResult {
  ...
  const txn: { pending: Pending | null; target: { price: number; rawPrice?: number; postable?: boolean; pct: number; at: number; index: number } | null } = { pending: null, target: null };
  ...
  function arm(price: number, at: number, index: number) {
    ...
    const d = opts.researchTpDeferral || opts.researchTpObserver ? Object.freeze({ at, index, episode, depth: positions.length, price,
    ...
    const tpPrice = checkBatchTp(positions, pct, price).tpPrice;
    if (maker) {
  ...
  function close(price: number, at: number, index: number, intent: Pending, exitFee = config.feeRate) {
    const before = positions, qty = before.reduce((v, p) => v + p.qty, 0), net = pnl(before, price, exitFee), entry = avg();
    realized += net;
    ...
    executions.push({ ...(maker ? { exitFeeRate: exitFee, executionMode: exitFee === maker.makerFee ? "maker" : "market" } : {}), kind: "close", decisionAt: intent.decisionAt, fillAt: at, decisionIndex: intent.decisionIndex, fillIndex: index, price, qty, reason: intent.reason });
  ...
  for (let i = opts.startIdx; i < end; i++) {
    ...
    if (positions.length && txn.target && (!maker || !txn.pending && txn.target.postable) && txn.target.at <= c.ts && txn.target.index < i && !gap && params.tpExecutionModel === "resting_touch" && c.high >= txn.target.price) {
    ...
    if (positions.length && (!maker || !txn.pending)) {
      minPnlPct = Math.min(minPnlPct, (c.close / avg() - 1) * 100);
      if (txn.target && c.close >= (txn.target.rawPrice ?? txn.target.price)) schedule("close", txn.target.pct < config.tpPct ? "stale_tp" : "tp");
      ...
      if (!txn.pending && action?.enabled && now >= srCooldownUntil && ctx) {
        ...
        const plan = d?.partialExitPlan, resistance = d?.levels.nearestResistance;
        if (plan && resistance && canExecuteSRPartialAction({ contextHealthy: ctx.coverage.healthy, hasDecision: !!d,
          ...
          planProfitOk: !action.requirePlanProfit || plan.estimatedNetPnl > 0, resistanceOk: resistance.distPct <= action.resistanceBufferPct, keepOk: plan.keepRungs === action.keepRungs })) {
