...
import fs from "fs";
import { CONTROLS, VARIANTS, POLICIES, PARENT, AgeHighController, config, size } from "./age10-gate-policy";
...
import { validate } from "./age10-gate-policy";
...
assert.equal(VARIANTS.length, 3); assert.equal(CONTROLS.length, 6); assert.equal(new Set(POLICIES.map(p => p.id)).size, 9);
...
const tape = series(cs), probeCfg = structuredClone(cfg); probeCfg.srSupportReopenAction.enabled = false;
let funding = 0;
tape.marketInputs = { snapshot: () => ({ pulse: { ...emptyReplayPulse(), fdBnNow: funding }, sources: {} }) } as any;
const o = new GateObserver(tape, probeCfg);
tape.rsi1H[9] = 80;
o.observe({ before: [], after: [], event: { kind: "close", reason: "tp", fillIndex: 10, decisionIndex: 1, fillAt: cs[10].endTs } } as any);
assert.equal(o.lastTp!.rsiIndex, 9, "resting TP uses last known pre-fill RSI, not target-arm or current candle RSI");
const permitted = structuredClone(probeCfg); permitted.tpCooldown.enabled = false; permitted.deepAddStressGuard.enabled = false;
const d = (index: number, nextDepth = 1, priceDropOk = false): any => ({ index, at: cs[index].endTs, episode: 1, nextDepth, priceDropOk, price: 100 });
o.clock(11); assert(o.entry(d(11), permitted).hotWouldBlock);
o.clock(40); assert(!o.entry(d(40), permitted).hotWouldBlock, "30min exact boundary releases");
o.inventory = Array.from({ length: 5 }, (_, level) => ({ id: String(level), level, entryTime: T, entryPrice: 100, qty: 1, notional: 100 }));
o.clock(41); funding = -.0001;
assert(o.entry(d(41, 6), permitted).deepWouldBlock);
assert(!o.entry(d(41, 6, true), permitted).deepWouldBlock, "genuine drop still bypasses funding timer gate");
funding = 0; assert(!o.entry(d(41, 6), permitted).deepWouldBlock);
validate(JSON.parse(fs.readFileSync("research-inputs/age10-gate-factorial-2026-09-11.json", "utf8")).definition);
console.log("L15 tests passed: frozen 3 definitions, six controls, config isolation, thresholds, lag, maxima, future-prefix, same-price rearming, all target caps, fees, cooldown and gate observer timing");
