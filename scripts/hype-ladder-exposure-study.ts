...
import type { ExposureSpec, ExposureVariant } from "./ladder-exposure-policy";
...
async function main() {
  const root = path.resolve(__dirname, ".."), definition = "research-inputs/sr-pulse-encounters/ladder-exposure-controls-2026-09-05.json";
  const read = (file: string) => JSON.parse(fs.readFileSync(path.resolve(root, file), "utf8"));
  const spec: ExposureSpec = read(definition), cfg: BotConfig = read("bot-config.json");
  ...
  console.log("[manifest] hash preserved inputs and frozen experiment; baseline engine change limited to research veto seam");
  const inputs: any[] = [];
  ...
  const sourceFiles = [...new Set<string>([...archived.sources.map((s: any) => s.file), definition, "scripts/hype-ladder-exposure-study.ts",
    "scripts/ladder-exposure-policy.ts", "scripts/ladder-exposure-metrics.ts", "scripts/ladder-exposure-tests.ts", "package-lock.json", "tsconfig.json"])];
  const sources = sourceFiles.map(file => ({ file, sha256: hash(fs.readFileSync(path.join(root, file))) }));
  const out = path.resolve(root, process.env.EXPOSURE_OUT ?? `backtests/hype/${spec.id}`), rel = path.relative(path.join(root, "backtests"), out);
  ...
  json("manifest.json", { at: new Date().toISOString(), node: process.version, spec, inputs, sources,
    ...
    engineChange: "Optional veto after existing entry gates/affordability; no-hook/no-op behavior must reproduce archived complete result digests",
    caveats: ["Previously mined development data", "No exact live/maker/funding/rounding model", "Pulse rules inactive for unknown historical required inputs", "Four overlapping baseline cases, not independent samples", "No live modifications"] });
  const core = await import("./hype-freerun-canonical-replay"), { runCausalLongReplay } = await import("./replay-causal-engine");
  const { exposureVariants, exposureVeto } = await import("./ladder-exposure-policy");
  ...
  const variants = exposureVariants(spec); assert.equal(variants.length, spec.variantCount);
  ...
  for (const prior of [...expected].sort((a: any, b: any) => Number(!a.id.startsWith("hl_window")) - Number(!b.id.startsWith("hl_window")))) {
    ...
    const run = (variant: ExposureVariant | null) => {
      ...
      const r = runCausalLongReplay({ ...params, id: variant ? label : prior.id }, s, { ...opts, researchAddVeto: d => {
        ...
        if (d.nextDepth >= variant.depth) opportunities++;
        const answer = exposureVeto(variant, d, spec); if (answer.unknown) unknown++;
      ...
      const data = { model: prior.id, variant: variant?.id ?? "baseline", family: variant?.family ?? "baseline", depth: variant?.depth ?? null,
        digest, opportunities, vetoMinutes, vetoEpisodes: episodes.size, unknown, maxKnownAt, metrics };
    ...
    for (const [n, variant] of variants.entries()) {
      ...
      flatRows.push({ model: prior.id, variant: variant.id, ...metrics, ...changes, vetoMinutes: result.vetoMinutes, vetoEpisodes: result.vetoEpisodes, unknown: result.unknown });
      monthRows.push(...deltaMonths.map(m => ({ model: prior.id, variant: variant.id, ...m })));
      ...
      console.log(JSON.stringify({ model: prior.id, variant: variant.id, pnlDelta: delta.pnlDelta, ddReductionPp: delta.ddReductionPp,
        grossLossReductionPct: delta.grossLossReductionPct, worstMonthDelta: delta.worstMonthDelta, vetoEpisodes: result.vetoEpisodes }));
    }
  }
  assert.equal(results.length, 4 * spec.variantCount);
  const ranking = variants.map(v => ({ ...v, ...qualifyExposure(results.filter(r => r.variant === v.id), spec) }))
  ...
  json("validation.json", { allFourBaselineDigestsMatched: true, causalFillsAndKnownResistance: true, sourceAndInputHashesUnchanged: true,
