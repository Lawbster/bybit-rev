...
export function auditSoftStalePath(cs: Candle[], events: ResearchInventoryEvent[], targets: R[], observations: R[], x: R, spec: R, cfg: R, tape: R) {
  const scheduledClose = new Set(events.filter(e => e.event.kind === "close" && e.event.fillAt === cs[e.event.fillIndex!].ts).map(e => e.event.decisionIndex));
  if (x.pendingAtEnd?.kind === "close") scheduledClose.add(x.pendingAtEnd.decisionIndex);
  ...
  for (let i = x.startIdx; i < x.endIdx; i++) {
    ...
    if (!inv.length || scheduledClose.has(i)) continue;
