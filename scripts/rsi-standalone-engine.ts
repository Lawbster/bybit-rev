/** I04 bounded standalone RSI expansion; old pinned I01 engine remains byte-for-byte intact.
...
export interface RsiRule {
  id: string; family: "rsi_extreme" | "rsi_center" | "clock"; side: "long" | "short"; exit: "fixed12h" | "indicator_or12h";
  timeframeMs: number; upper: number; mode: "into" | "back_out";
}
export type RsiResult = Omit<StandaloneResult, "rule"> & { rule: RsiRule };
const finite = (x: unknown): x is number => typeof x === "number" && Number.isFinite(x);
function validateRule(rule: RsiRule): void {
  if (!["rsi_extreme", "rsi_center", "clock"].includes(rule.family) || !["long", "short"].includes(rule.side)
    || !["into", "back_out"].includes(rule.mode) || !["fixed12h", "indicator_or12h"].includes(rule.exit)
    || ![5 * MINUTE, 15 * MINUTE, 30 * MINUTE, HOUR, 4 * HOUR].includes(rule.timeframeMs)
    || (rule.family === "rsi_extreme" ? ![60, 70, 80, 90, 95].includes(rule.upper)
      : rule.upper !== 50 || rule.mode !== "back_out" || rule.exit !== "fixed12h")) throw new Error("Outside frozen RSI grid");
}
export function rsiEntrySignal(rule: RsiRule, previous: FeatureBar | undefined, current: FeatureBar): boolean {
  validateRule(rule);
  ...
  const a = previous.feature.rsi14, b = current.feature.rsi14;
  if (!finite(a) || !finite(b) || a < 0 || a > 100 || b < 0 || b > 100) return false;
  const level = rule.family === "rsi_center" ? 50 : rule.side === "long" ? 100 - rule.upper : rule.upper;
...
export function runRsiStandalone(minutes: readonly Readonly<Candle>[], features: readonly FeatureBar[], rule: RsiRule, o: StandaloneOptions): RsiResult {
  validateRule(rule);
  ...
  for (const c of selected) {
    ...
    if (p && !exit) {
      ...
      const rsi = context?.current.feature.rsi14;
      const neutral = rule.exit === "indicator_or12h" && t > p.entryAt && finite(rsi)
        && (rule.side === "long" ? rsi >= 50 : rsi <= 50);
      if (timedOut || neutral) {
    ...
    if (clockTrigger || (rule.family !== "clock" && context && rsiEntrySignal(rule, context.previous, context.current))) {
      rawSignals++;
