/** Frozen AdxDmi-only standalone research. New local artifacts; no live/config writes. */
import fs from "fs";
...
import { runAdxDmiStandalone, adxDmiEntrySignal, type AdxDmiRule, type AdxDmiResult } from "./adx-dmi-standalone-engine";
import { computeAdxDmiValues, ADX_DMI_FORMULA_VERSION } from "../src/research/adx-dmi-features";
...
async function main() {
  ...
  assert.equal(spec.holdMs, 12 * HOUR); assert.equal(spec.strategyDefinitionCount, 540); assert.equal(spec.symbol, "HYPEUSDT");
  ...
  const sourceFiles = [definition, "scripts/adx-dmi-standalone-engine.ts", "scripts/hype-adx-dmi-standalone-study.ts",
    "src/research/adx-dmi-features.ts", "scripts/adx-dmi-standalone-tests.ts", "scripts/adx-dmi-standalone-results-check.ts", "scripts/adx-dmi-standalone-path-check.ts",
  ...
  const ruleKey = (r: AdxDmiRule) => featureKey(r.timeframeMs,[r.period]);
  let prefixChecks = 0;
  for (const tf of spec.timeframesMs as number[]) {
    ...
    for (const n of spec.periods as number[]) { const p=[n];
      const values=computeAdxDmiValues(closed,tf,p[0]);
      const rows=bars.bars.map((b,i)=>({...b,feature:{...base[i],...values[i],
        dmPeriod:p[0]}}));
      features.set(featureKey(tf,p),rows);
      if(n===14) values.forEach((v,i)=>{assert.equal(v.dmAtr,base[i].atr14);assert.equal(v.dmPlus,base[i].plusDI14);assert.equal(v.dmMinus,base[i].minusDI14);assert.equal(v.dmAdx,base[i].adx14);});
      for(const i of [800,Math.floor(rows.length/2),rows.length-3]) {
        ...
        assert.deepEqual(computeAdxDmiValues(prefix.bars.map(b=>b.candle),tf,p[0]).at(-1),values[i]);
        assert(bars.windowAt(at,i+1).ready); prefixChecks++;
      }
    }
    console.log(`[features] ${tf/MINUTE}m ${oldRows.length} bars; three ADX/DMI periods / nine prefixes passed; period14 shared math exact`);
  }
  const rules: AdxDmiRule[]=[];
  for(const timeframeMs of spec.timeframesMs) for(const period of spec.periods)
    for(const {mode,threshold} of spec.entryDefinitions) for(const side of spec.sides) for(const exit of spec.exits)
      rules.push({id:`adx_${timeframeMs/MINUTE}m_n${period}_${mode}_t${threshold}_${side}_${exit}`,
        family:"adx_dmi",timeframeMs,period,threshold,mode,side,exit});
  const controls: AdxDmiRule[]=spec.sides.map((side:"long"|"short")=>({id:`clock_${side}`,family:"clock",side,
    exit:"fixed12h",timeframeMs:HOUR,period:14,threshold:0,mode:"di_cross"}));
  assert.equal(rules.length,540); assert.equal(new Set(rules.map(r=>r.id)).size,540);
  ...
  const cache = new Map<string, AdxDmiResult>(), parity: any[] = [];
  for (const w of spec.windows) for (const delayMs of spec.executionDelaysMs) {
    for (const r of controls) {
      ...
      const fresh=runAdxDmiStandalone(minutes,baseFeatures.get(HOUR)!,r,options(w,delayMs));
  ...
  json(path.join(out, "manifest.json"), { spec, node: process.version, sources, inputs, minuteRows: minutes.length, seed,
    formulaVersion: RESEARCH_FEATURE_FORMULA_VERSION + "|" + ADX_DMI_FORMULA_VERSION, timingVersion: CLOSED_BAR_TIMING_VERSION,
  ...
  for (const w of spec.windows) for (const delayMs of spec.executionDelaysMs) for (const rule of [...controls, ...rules]) {
    ...
    const r = cache.get(key(w.id, delayMs, rule.id)) ?? runAdxDmiStandalone(minutes, rows, rule, options(w, delayMs));
    ...
    results.push({ window: w.id, start: w.start, end: w.end, delayMs, ...rule, ...r.stats,
      ...
      deltaVsOwnFixed12h: rule.exit === "indicator_or12h" ? r.stats.net - results.find(x => x.window === w.id && x.delayMs === delayMs && x.id === rule.id.replace("indicator_or12h", "fixed12h")).net : null, deltaVsUnfilteredDiCross: rule.mode==="di_strong" ? r.stats.net - results.find(x=>x.window===w.id&&x.delayMs===delayMs&&x.id===`adx_${rule.timeframeMs/MINUTE}m_n${rule.period}_di_cross_t0_${rule.side}_${rule.exit}`).net : null, stressNet: r.stats.net - r.stats.turnoverIncludingMarkedExit * spec.extraFixedPathCostBpsPerSide / 10000, open: r.open });
    ...
    if (rule.family !== "clock" && w.id === "full" && delayMs === 0 && r.trades.length) {
      ...
      assert(i>0); assert(adxDmiEntrySignal(rule,rows[i-1],rows[i],rows[i-2]));
      const f=(j:number)=>{const x=rows[j].feature as any;return {dmAtr:x.dmAtr,dmPlus:x.dmPlus,dmMinus:x.dmMinus,dmDx:x.dmDx,dmAdx:x.dmAdx,dmSpread:x.dmSpread};};
  ...
  assert.equal(results.length, 2168);
  ...
  for (const rule of rules) {
    ...
    assert.deepEqual(runAdxDmiStandalone(minutes, rows, rule, opts), runAdxDmiStandalone(
  ...
  json(path.join(out, "validation.json"), { missingMinutes: 0, overlapParityCases: parity.length, exactSavedOverlap: true,
    actualDataFeaturePrefixChecks: prefixChecks, period14SharedMathExact: true, actualDataEnginePrefixChecks: enginePrefixChecks,
    strategyDefinitions: rules.length, newDefinitions: 540, repeatedDefinitions: 0, controls: 2, strategyCases: 2160, controlCases: 8,
