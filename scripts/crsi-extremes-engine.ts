/** Additive stage-2 CRSI runner; old pinned I01 engine remains byte-for-byte intact.
 * Minute accounting follows I01, with explicit timeframe and crossing definitions.
...
export interface CrsiRule {
  id: string; family: "crsi_extreme" | "clock"; side: "long" | "short"; exit: "fixed12h";
  timeframeMs: number; upper: number; mode: "into" | "back_out";
}
export type CrsiResult = Omit<StandaloneResult, "rule"> & { rule: CrsiRule };
const finite = (x: unknown): x is number => typeof x === "number" && Number.isFinite(x);
function validateRule(rule: CrsiRule): void {
  if (!["crsi_extreme", "clock"].includes(rule.family) || !["long", "short"].includes(rule.side)
    || !["into", "back_out"].includes(rule.mode) || rule.exit !== "fixed12h"
    || ![15 * MINUTE, 30 * MINUTE, HOUR].includes(rule.timeframeMs)
    || ![80, 90, 95].includes(rule.upper)) throw new Error("Outside frozen CRSI grid");
}
export function crsiEntrySignal(rule: CrsiRule, previous: FeatureBar | undefined, current: FeatureBar): boolean {
  validateRule(rule);
  ...
  const a = previous.feature.crsi, b = current.feature.crsi;
  if (!finite(a) || !finite(b) || a < 0 || a > 100 || b < 0 || b > 100) return false;
  const level = rule.side === "long" ? 100 - rule.upper : rule.upper;
...
export function runCrsiStandalone(minutes: readonly Readonly<Candle>[], features: readonly FeatureBar[], rule: CrsiRule, o: StandaloneOptions): CrsiResult {
  validateRule(rule);
  ...
  for (const c of selected) {
    ...
    if (p && !exit) {
      ...
      if (timedOut) {
        exit = { decisionAt: t, due: t + o.delayMs, reason: "timeout" };
        if (o.delayMs === 0) closeAt(c);
      }
    }
    const clockTrigger = rule.family === "clock" && (t % (12 * HOUR) === 0 || lastClosedAt === t);
    if (clockTrigger || (rule.family !== "clock" && context && crsiEntrySignal(rule, context.previous, context.current))) {
      rawSignals++;
