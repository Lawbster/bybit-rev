/** I06 standalone MACD. I01 minute accounting preserved; no live mutation. */
import type { Candle } from "../src/fetch-candles";
import type { ResearchFeature } from "../src/research/indicator-features";
import type { MacdValues } from "../src/research/macd-features";
...
export interface MacdRule {
  id: string; family: "macd" | "clock"; side: "long" | "short"; exit: "fixed12h" | "indicator_or12h";
  timeframeMs: number; fastPeriod: number; slowPeriod: number; signalPeriod: number;
  mode: "signal_cross" | "signal_trend" | "signal_counter" | "zero_cross" | "hist_turn";
}
export type MacdResult = Omit<StandaloneResult, "rule"> & { rule: MacdRule };
export interface MacdFeature extends ResearchFeature, MacdValues { macdFastPeriod: number; macdSlowPeriod: number; macdSignalPeriod: number }
export type MacdFeatureBar = Omit<FeatureBar, "feature"> & { feature: MacdFeature };
const finite = (x: unknown): x is number => typeof x === "number" && Number.isFinite(x);
function validateRule(r: MacdRule): void {
  if (!["macd","clock"].includes(r.family) || !["long","short"].includes(r.side)
    || !["fixed12h","indicator_or12h"].includes(r.exit)
    || !["signal_cross","signal_trend","signal_counter","zero_cross","hist_turn"].includes(r.mode)
    || ![5*MINUTE,15*MINUTE,30*MINUTE,HOUR,4*HOUR].includes(r.timeframeMs)
    || ![[6,13,5],[12,26,9],[24,52,18]].some(p=>p[0]===r.fastPeriod&&p[1]===r.slowPeriod&&p[2]===r.signalPeriod)
    || (r.family==="clock"&&(r.exit!=="fixed12h"||r.timeframeMs!==HOUR||r.fastPeriod!==12||r.slowPeriod!==26||r.signalPeriod!==9||r.mode!=="signal_cross")))
    throw new Error("Outside frozen MACD grid");
}
function at(r: MacdRule,b: FeatureBar | undefined): Partial<MacdFeature> | null {
  if (!b || b.barEnd!==b.candle.timestamp+r.timeframeMs || b.availableAt!==b.barEnd || b.feature.timestamp!==b.candle.timestamp) return null;
  const f=b.feature as Partial<MacdFeature>;
  return f.macdFastPeriod===r.fastPeriod&&f.macdSlowPeriod===r.slowPeriod&&f.macdSignalPeriod===r.signalPeriod?f:null;
}
export function macdEntrySignal(r: MacdRule,prev: FeatureBar | undefined,cur: FeatureBar,older?: FeatureBar): boolean {
  validateRule(r);
  if(r.family==="clock") return cur.barEnd%(12*HOUR)===0;
  if(!prev||prev.barEnd!==cur.candle.timestamp) return false;
  const a=at(r,prev),b=at(r,cur); if(!a||!b) return false;
  const sign=r.side==="long"?1:-1;
  if(r.mode==="zero_cross") return finite(a.macdLine)&&finite(b.macdLine)&&sign*a.macdLine<=0&&sign*b.macdLine>0;
  if(!finite(a.macdHist)||!finite(b.macdHist)) return false;
  if(r.mode==="hist_turn") {
    const z=at(r,older);
    return !!z&&older!.barEnd===prev.candle.timestamp&&finite(z.macdHist)
      &&sign*(a.macdHist-z.macdHist)<=0&&sign*(b.macdHist-a.macdHist)>0&&sign*b.macdHist<0;
  }
  if(!(sign*a.macdHist<=0&&sign*b.macdHist>0)) return false;
  if(r.mode==="signal_cross") return true;
  return finite(b.macdLine)&&(r.mode==="signal_trend"?sign*b.macdLine>0:sign*b.macdLine<0);
}
export function macdExitSignal(r: MacdRule,cur: FeatureBar | undefined): boolean {
  if(r.exit==="fixed12h"||r.family==="clock") return false;
  const b=at(r,cur); if(!b) return false;
  const v=r.mode==="zero_cross"?b.macdLine:b.macdHist; if(!finite(v)) return false;
  const signed=(r.side==="long"?1:-1)*v;
...
export function runMacdStandalone(minutes: readonly Readonly<Candle>[], features: readonly FeatureBar[], rule: MacdRule, o: StandaloneOptions): MacdResult {
  validateRule(rule);
  ...
  features.forEach((current, i) => {
    ...
    if (rule.family !== "clock" && !at(rule, current)) throw new Error("MACD parameter provenance mismatch");
  ...
  for (const c of selected) {
    ...
    if (p && !exit) {
      ...
      const neutral = t > p.entryAt && macdExitSignal(rule, context?.current);
      if (timedOut || neutral) {
    ...
    if (clockTrigger || (rule.family !== "clock" && context && macdEntrySignal(rule, context.previous, context.current, context.older))) {
      rawSignals++;
