...
// independently weighted Wilder14 averages; no shared RSI implementation.
function referenceRsi(candles: any[], tf: number): Map<number, number | null> {
  ...
  return new Map(starts.map((at, i) => [at, values[i]]));
}
function crossed(r: any, previous: number | null | undefined, current: number | null | undefined): boolean {
  if (previous == null || current == null) return false;
  const threshold = r.side === "long" ? 100 - r.upper : r.upper;
  if (r.mode === "into") return r.side === "long" ? previous > threshold && current <= threshold : previous < threshold && current >= threshold;
  return r.side === "long" ? previous <= threshold && current > threshold : previous >= threshold && current < threshold;
}

async function main(): Promise<void> {
  assert.equal(process.argv.length, 3, "Usage: npx ts-node scripts/rsi-standalone-results-check.ts ARTIFACT_DIRECTORY");
  ...
  const expectedRules = spec.timeframesMs.flatMap((timeframeMs: number) => spec.upperLevels.flatMap((upper: number) =>
    spec.entryModes.flatMap((mode: string) => spec.sides.flatMap((side: string) => spec.exits.map((exit: string) => ({
      family: "rsi_extreme", side, exit, timeframeMs, upper, mode, id: `rsi_${timeframeMs / M}m_${upper}_${mode}_${side}_${exit}` }))))));
  expectedRules.push(...spec.timeframesMs.flatMap((timeframeMs: number) => spec.sides.map((side: string) => ({
    family: "rsi_center", side, exit: "fixed12h", timeframeMs, upper: 50, mode: "back_out", id: `rsi_${timeframeMs / M}m_center50_${side}_fixed12h` }))));
  expectedRules.push(...spec.sides.map((side: string) => ({ family: "clock", side, exit: "fixed12h", timeframeMs: H, upper: 50, mode: "back_out", id: `clock_${side}` })));
  ...
  assert.equal(expected.size, 848); assert.equal(summaries.length, expected.size);
  ...
  const referenceByTf = new Map<number, Map<number, number | null>>();
  for (const tf of spec.timeframesMs) {
    const values = referenceRsi(candles, tf); referenceByTf.set(tf, values);
  ...
  for (const [key, r] of byCase) {
    ...
    const firstExit = (entered: number): Exit | null => {
      ...
      if (r.exit === "indicator_or12h") {
        const ref = referenceByTf.get(r.timeframeMs)!;
        // Only subsequent closed boundaries; do not reuse entry's RSI value.
        for (let at = (Math.floor(entered / r.timeframeMs) + 1) * r.timeframeMs;
          ...
          if (value != null && (r.side === "long" ? value >= 50 : value <= 50)) {
    ...
    const validateEntry = (trade: any, label: string) => {
      ...
      if (r.family !== "clock") {
        ...
        const references = referenceByTf.get(r.timeframeMs)!;
        assert.ok(crossed(r, references.get(signal - 2 * r.timeframeMs), references.get(signal - r.timeframeMs)), `${label} independently reconstructed RSI crossing`);
      }
    ...
    for (const [i, t] of rows.entries()) {
      ...
      if (t.entryFeature !== null) {
        ...
        const expectedRsi = referenceByTf.get(r.timeframeMs)!.get(featureStart);
        if (expectedRsi == null) assert.equal(t.entryFeature.rsi14, null);
        else near(t.entryFeature.rsi14, expectedRsi, `${label} independently calculated RSI`, 1e-8);
    ...
    if (r.family === "clock") {
      ...
      const reference = referenceByTf.get(r.timeframeMs)!;
  ...
  // Four repeated RSI definitions and the two shared clocks must retain their
  ...
  let overlapCases = 0;
  for (const r of summaries.filter(r => r.family === "clock" || r.timeframeMs === H && r.upper === 70 && r.mode === "back_out")) {
    const oldId = r.family === "clock" ? r.id : `rsi_recovery_${r.side}_${r.exit}`;
  ...
  assert.equal(ranking.length, 210);
  ...
  for (const trace of traces) {
    ...
    const ref = referenceByTf.get(r.timeframeMs)!;
    near(trace.previousRsi, ref.get(first.signalAt - 2 * r.timeframeMs)!, "trace previous RSI");
    near(trace.currentRsi, ref.get(first.signalAt - r.timeframeMs)!, "trace current RSI");
  ...
  const report = { version: 1, verifiedAt: new Date().toISOString(), passed: true,
    ...
    checks: ["independent RSI formulas for all five timeframes and exact into/back-out crossings",
    ...
    limitations: ["No strategy engine is imported; independent scalar RSI formulas/crossing enumeration and ledger accounting check this fixed-timeout/neutral-exit study only.",
      "No untouched holdout or RSI combination tests; shortlist is descriptive, not live approval.",
