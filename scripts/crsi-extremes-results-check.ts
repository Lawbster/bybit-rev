...
async function main(): Promise<void> {
  assert.equal(process.argv.length, 3, "Usage: npx ts-node scripts/crsi-extremes-results-check.ts ARTIFACT_DIRECTORY");
  ...
  assert.equal(spec.symbol, "HYPEUSDT"); assert.deepEqual(spec.timeframesMs, [15 * M, 30 * M, H]);
  assert.deepEqual(spec.upperLevels, [80, 90, 95]); assert.deepEqual(spec.entryModes, ["into", "back_out"]);
  assert.deepEqual(spec.sides, ["long", "short"]); assert.equal(spec.exit, "fixed12h");
  ...
  const artifactNames = ["manifest.json", "results.json", "monthly.json", "trades.jsonl", "context-controls.json", "ranking.json", "validation.json", "overlap-parity.json", "causal-traces.json"];
  ...
  const expectedRules = spec.timeframesMs.flatMap((timeframeMs: number) => spec.upperLevels.flatMap((upper: number) =>
    spec.entryModes.flatMap((mode: string) => spec.sides.map((side: string) => ({ family: "crsi_extreme", side,
      exit: "fixed12h", timeframeMs, upper, mode, id: `crsi_${timeframeMs / M}m_${upper}_${mode}_${side}` })))));
  expectedRules.push(...spec.sides.map((side: string) => ({ family: "clock", side, exit: "fixed12h", timeframeMs: H, upper: 80, mode: "back_out", id: `clock_${side}` })));
  ...
  assert.equal(expected.size, 152); assert.equal(summaries.length, expected.size);
  ...
  for (const tf of spec.timeframesMs) {
    const values = referenceCrsi(candles, tf); referenceByTf.set(tf, values);
  ...
  for (const [key, r] of byCase) {
    ...
    const validateEntry = (trade: any, label: string) => {
      ...
      if (r.family !== "clock") {
        ...
        assert.ok(crossed(r, references.get(signal - 2 * r.timeframeMs), references.get(signal - r.timeframeMs)), `${label} independently reconstructed CRSI crossing`);
      }
    ...
    for (const [i, t] of rows.entries()) {
      ...
      assert.equal(t.reason, "timeout"); assert.equal(decided, t.entryAt + spec.holdMs, `${label} timeout from actual fill`);
      ...
      if (t.entryFeature !== null) {
        ...
        const expectedCrsi = referenceByTf.get(r.timeframeMs)!.get(featureStart);
    ...
    // Fixed holds make the busy interval known from signal+both execution delays.
    ...
    for (const at of [...opportunities].sort((a, b) => a - b)) {
      ...
      else { accepted.push(at); freeAt = at + spec.holdMs + 2 * r.delayMs; }
    }
    ...
    const pendingExit = !!open && open.entryAt + spec.holdMs < stop && open.entryAt + spec.holdMs + r.delayMs >= stop;
  ...
  for (const [key, r] of byCase) {
    ...
    assert.equal(r.deltaVsOwnFixed12h, null, "All CRSI expansion exits are fixed12h");
  ...
  // Two repeated CRSI definitions and the two shared clocks must retain their
  ...
  let overlapCases = 0;
  for (const r of summaries.filter(r => r.family === "clock" || r.timeframeMs === H && r.upper === 80 && r.mode === "back_out")) {
    const oldId = r.family === "clock" ? r.id : `crsi_recovery_${r.side}_fixed12h`;
  ...
  const report = { version: 1, verifiedAt: new Date().toISOString(), passed: true,
    ...
    checks: ["independent CRSI formulas for all three timeframes and exact into/back-out crossings",
      "every eligible flat signal and occupied skip independently enumerated", "terminal pending decisions independently derived",
      "full minute close/adverse equity drawdown independently reconstructed", "16 prior saved ledger/monthly/stat overlap cases",
      ...
      "same-side clock deltas; all policy exits remain fixed12h", "cash and buy-and-hold contextual controls"],
    limitations: ["No strategy engine is imported; independent scalar CRSI formulas/crossing enumeration and ledger accounting check this fixed-timeout study only.",
