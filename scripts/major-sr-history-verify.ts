...
async function main(){const key=process.argv[2],dir=jobDirectory(process.cwd(),key),plan:Plan=read(`${dir}/plan.json`),state=read(`${dir}/state.json`),d=plan.card.definition;
 ...
 assert.equal(map.key,key);assert.deepEqual(events,read(`${d.archive}/output/240m-events.json`));
 ...
 assert.deepEqual(direct.events,events);assert.deepEqual(direct.snapshot(),read(`${d.archive}/output/240m-final.json`));
 ...
 const embedded=JSON.parse(html.match(/<script id="map-data" type="application\/json">([\s\S]*?)<\/script>/)![1]);assert.deepEqual(embedded.levels,map.levels);assert.deepEqual(embedded.frames,map.frames);
