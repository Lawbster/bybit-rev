/** Read-only I04 report supplement. No new signals, exits or strategy runs. */
...
async function main(): Promise<void> {
  ...
  const out = path.resolve(root, process.argv[2] ?? "backtests/hype/hype-rsi-standalone-2026-09-06");
  ...
  const ids = ["clock_long", "clock_short", ...read("shortlist.json").topFiveIds, "rsi_240m_center50_long_fixed12h"];
  ...
  for (const id of [...new Set<string>(ids)]) for (const window of ["full", "recent"]) {
    ...
    assert.ok(selected.length > 0, `No completed trades for ${window}/${id}`);
