...
// Independent OHLCV aggregation and direct window math; no feature/strategy imports.
export function referenceVolumeFlow(candles:any[],tf:number,family:string,n:number):Map<number,any>{
  const step=tf/M,bars:any[]=[];
  for(let i=0;i+step<=candles.length;i+=step){
    let high=-Infinity,low=Infinity,volume=0;
    for(let j=i;j<i+step;j++){high=Math.max(high,candles[j].high);low=Math.min(low,candles[j].low);volume+=candles[j].volume;}
    bars.push({ts:candles[i].ts,high,low,close:candles[i+step-1].close,volume});
  }
  let obv=0;
  return new Map(bars.map((b,i):[number,any]=>{
    if(i)obv+=Math.sign(b.close-bars[i-1].close)*b.volume;
    const x:any={vfRawObv:family==="obv"?obv:null,vfPositive:null,vfNegative:null,vfVolume:null,
      vfNumerator:null,vfDenominator:null,vfValue:null,vfSigned:null};
    if(i<(family==="cmf"?n-1:n))return[b.ts,x];
    let pos=0,neg=0,volume=0,numerator=0;
    for(let j=i-n+1;j<=i;j++){
      const c=bars[j];volume+=c.volume;
      if(family==="obv")numerator+=Math.sign(c.close-bars[j-1].close)*c.volume;
      else if(family==="mfi"){
        const typical=(c.high+c.low+c.close)/3,p=bars[j-1],previous=(p.high+p.low+p.close)/3;
        if(typical>previous)pos+=typical*c.volume;
        else if(typical<previous)neg+=typical*c.volume;
      }else numerator+=(c.high===c.low?0:((c.close-c.low)-(c.high-c.close))/(c.high-c.low))*c.volume;
    }
    x.vfVolume=volume;
    if(family==="mfi"){x.vfPositive=pos;x.vfNegative=neg;numerator=pos-neg;}
    x.vfNumerator=numerator;x.vfDenominator=family==="mfi"?pos+neg:volume;
    if(x.vfDenominator>0){
      x.vfSigned=numerator/x.vfDenominator;
      x.vfValue=family==="mfi"?100*pos/x.vfDenominator:x.vfSigned;
    }
    return[b.ts,x];
  }));
}
function crossed(r:any,a:any,b:any,_z?:any):boolean{
  if(a?.vfSigned==null||b?.vfSigned==null)return false;
  const sign=r.side==="long"?1:-1,p=a.vfSigned*sign,c=b.vfSigned*sign;
  if(r.mode==="trend")return p<=r.threshold&&c>r.threshold;
  if(r.mode==="into")return p>=-r.threshold&&c<-r.threshold;
  return p<=-r.threshold&&c>-r.threshold;
}
function zeroExit(r:any,b:any):boolean{
  if(b?.vfSigned==null)return false;const v=(r.side==="long"?1:-1)*b.vfSigned;
  return r.mode==="trend"?v<=0:v>=0;
}

async function main(): Promise<void> {
  assert.equal(process.argv.length, 3, "Usage: npx ts-node scripts/volume-flow-standalone-results-check.ts ARTIFACT_DIRECTORY");
  ...
  const definition = "research-inputs/indicators/volume-flow-standalone-2026-09-07.json";
  ...
  for(const timeframeMs of spec.timeframesMs)for(const p of spec.parameters)
    for(const {mode,thresholds} of p.levels)for(const threshold of thresholds)for(const side of spec.sides)for(const exit of spec.exits)
      expectedRules.push({family:p.family,side,exit,timeframeMs,period:p.period,threshold,mode,
        id:`${p.family}_${timeframeMs/M}m_n${p.period}_${mode}_t${threshold}_${side}_${exit}`});
  expectedRules.push(...spec.sides.map((side:string)=>({family:"clock",side,exit:"fixed12h",timeframeMs:H,
    period:14,threshold:0,mode:"trend",id:`clock_${side}`})));
  ...
  assert.equal(expected.size, 7208); assert.equal(summaries.length, expected.size);
  ...
  const referenceKey=(r:any)=>`${r.timeframeMs}:${r.family}:${r.period}`;
  for(const tf of spec.timeframesMs)for(const p of spec.parameters){
    const values=referenceVolumeFlow(candles,tf,p.family,p.period);referenceByTf.set(`${tf}:${p.family}:${p.period}`,values);
  ...
  for (const [key, r] of byCase) {
    ...
    const firstExit = (entered: number): Exit | null => {
      ...
      if (r.exit === "indicator_or12h") {
        ...
        // Only subsequent closed boundaries; do not reuse entry's OBV/MFI/CMF value.
    ...
    const validateEntry = (trade: any, label: string) => {
      ...
      if (r.family !== "clock") {
        ...
        assert.ok(crossed(r, references.get(signal - 2 * r.timeframeMs), references.get(signal - r.timeframeMs), references.get(signal - 3*r.timeframeMs)), `${label} independently reconstructed OBV/MFI/CMF crossing`);
      }
    ...
    for (const [i, t] of rows.entries()) {
      ...
      if (t.entryFeature !== null) {
        ...
        if(r.family!=="clock") {
          ...
          assert.equal(t.entryFeature.vfPeriod,r.period);assert.equal(t.entryFeature.vfFamily,r.family==="obv"?1:r.family==="mfi"?2:3);
        }
  ...
  assert.equal(ranking.length, 1800);
  ...
  assert.deepEqual(shortlist.byFamily,["obv","mfi","cmf"].map(family=>({family,
  ...
  for(const [k,v]of Object.entries({missingMinutes:0,overlapParityCases:8,exactSavedOverlap:true,
    actualDataFeaturePrefixChecks:135,actualDataEnginePrefixChecks:1800,strategyDefinitions:1800,newDefinitions:1800,
    repeatedDefinitions:0,controls:2,strategyCases:7200,controlCases:8,inputAndSourceHashesUnchanged:true,liveChanges:0,fundingIncluded:false}))
    assert.deepEqual(validation[k],v,`Validation ${k}`);
  assert.deepEqual(validation.passedResearchScreen,ranking.filter(r=>r.passesResearchScreen).map(r=>r.id));
  ...
  const report = { version: 1, verifiedAt: new Date().toISOString(), passed: true,
    ...
    checks: ["independent OBV/MFI/CMF formulas for all five timeframes; normalized OBV, typical-price MFI, close-location CMF; trend/into/recovery crossings",
    ...
    limitations: ["No strategy engine is imported; independent OHLC aggregation and direct-window OBV/MFI/CMF formulas/crossing enumeration and ledger accounting check this fixed-timeout/neutral-exit study only.",
      "No untouched holdout or OBV/MFI/CMF combination tests; shortlist is descriptive, not live approval.",
