...
// Independent anchored slices / prior-slot means; no feature/strategy imports.
function referenceVwapVolume(candles:any[],tf:number):Map<number,any>{
  const step=tf/M,D=86400000,W=7*D,bars:any[]=[];
  for(let i=0;i+step<=candles.length;i+=step){
    let high=-Infinity,low=Infinity,volume=0,turnover=0;
    for(let j=i;j<i+step;j++){const c=candles[j];high=Math.max(high,c.high);low=Math.min(low,c.low);volume+=c.volume;turnover+=c.turnover;}
    bars.push({ts:candles[i].ts,high,low,close:candles[i+step-1].close,volume,turnover});
  }
  const index=new Map<number,number>(bars.map((b,i)=>[b.ts,i]));
  return new Map(bars.map((b,i):[number,any]=>{
    const day=Math.floor(b.ts/D)*D,date=new Date(b.ts),daysSinceMonday=(date.getUTCDay()+6)%7,week=day-daysSinceMonday*D;
    const vwap=(at:number)=>{const first=index.get(at);if(first===undefined)return null;
      let v=0,q=0;for(let j=first;j<=i;j++){v+=bars[j].volume;q+=bars[j].turnover;}return v>0?q/v:null;};
    const d=vwap(day),w=vwap(week);
    let rolling:number|null=null,timed:number|null=null;
    if(i>=20){const volumes=bars.slice(i-20,i).map(c=>c.volume),mean=volumes.reduce((a,c)=>a+c,0)/20;if(mean>0)rolling=b.volume/mean;}
    const priorSlots=Array.from({length:20},(_,j)=>index.get(b.ts-(20-j)*D));
    if(priorSlots.every(j=>j!==undefined)){const mean=priorSlots.reduce((a,j)=>a+bars[j!].volume,0)/20;if(mean>0)timed=b.volume/mean;}
    return[b.ts,{vvDayStart:day,vvWeekStart:week,vvDay:d,vvWeek:w,
...
function crossed(r:any,a:any,b:any,_z?:any):boolean{
  if(!a||!b)return false;const sign=r.side==="long"?1:-1;
  if(r.family==="vwap"){
    const anchor=r.reference==="day"?"vvDayStart":"vvWeekStart",field=r.reference==="day"?"vvDayDistance":"vvWeekDistance";
    if(a[anchor]!==b[anchor]||a[field]==null||b[field]==null)return false;
    const p=sign*a[field],c=sign*b[field];
    if(r.mode==="trend")return p<=r.threshold&&c>r.threshold;
    return r.mode==="into"?p>=-r.threshold&&c<-r.threshold:p<=-r.threshold&&c>-r.threshold;
  }
  if(b.vvChange==null||!(r.mode==="follow"?sign*b.vvChange>0:sign*b.vvChange<0))return false;
  if(r.family==="bar_direction")return true;
  const field=r.reference==="rolling20"?"vvRolling20":"vvTime20";
  return a[field]!=null&&b[field]!=null&&a[field]<=r.threshold&&b[field]>r.threshold;
}
function zeroExit(r:any,b:any):boolean{
  if(!b)return false;
  if(r.family==="vwap"){const v=r.reference==="day"?b.vvDayDistance:b.vvWeekDistance;if(v==null)return false;
    const signed=(r.side==="long"?1:-1)*v;return r.mode==="trend"?signed<=0:signed>=0;}
  const v=r.reference==="rolling20"?b.vvRolling20:b.vvTime20;return v!=null&&v<=1;
}
function idOf(r:any):string{return r.family==="clock"?`clock_${r.side}`:`${r.family}_${r.timeframeMs/M}m_${r.reference}_${r.mode}_t${r.threshold}_${r.side}_${r.exit}`;}
function fixedId(r:any):string{return idOf({...r,exit:"fixed12h",reference:r.family==="bar_direction"?"rolling20":r.reference});}
function plainId(r:any):string|null{return r.family==="rvol"?idOf({...r,family:"bar_direction",threshold:0,reference:r.exit==="fixed12h"?"rolling20":r.reference}):null;}

async function main(): Promise<void> {
  assert.equal(process.argv.length, 3, "Usage: npx ts-node scripts/vwap-volume-standalone-results-check.ts ARTIFACT_DIRECTORY");
  ...
  assert.deepEqual(spec.vwapReferences,["day","week"]);assert.deepEqual(spec.volumeReferences,["rolling20","time20"]);
  assert.deepEqual(spec.vwapModes,[{mode:"trend",thresholds:[0,.5,1,2]},{mode:"into",thresholds:[.5,1,2]},{mode:"recovery",thresholds:[.5,1,2]}]);
  assert.deepEqual(spec.volumeThresholds,[1.5,2,3]);assert.deepEqual(spec.volumeModes,["follow","fade"]);
  assert.equal(spec.strategyDefinitionCount,640);assert.equal(spec.priceControlDefinitionCount,60);assert.equal(spec.totalDefinitionSlots,700);
  ...
  const add=(r:any)=>expectedRules.push({...r,id:idOf(r)});
  for(const timeframeMs of spec.timeframesMs)for(const mode of spec.volumeModes)for(const side of spec.sides){
    add({family:"bar_direction",timeframeMs,reference:"rolling20",threshold:0,mode,side,exit:"fixed12h"});
    for(const reference of spec.volumeReferences)add({family:"bar_direction",timeframeMs,reference,threshold:0,mode,side,exit:"indicator_or12h"});
  }
  for(const timeframeMs of spec.timeframesMs)for(const reference of spec.vwapReferences)for(const {mode,thresholds}of spec.vwapModes)
    for(const threshold of thresholds)for(const side of spec.sides)for(const exit of spec.exits)add({family:"vwap",timeframeMs,reference,mode,threshold,side,exit});
  for(const timeframeMs of spec.timeframesMs)for(const reference of spec.volumeReferences)for(const threshold of spec.volumeThresholds)
    for(const mode of spec.volumeModes)for(const side of spec.sides)for(const exit of spec.exits)add({family:"rvol",timeframeMs,reference,threshold,mode,side,exit});
  for(const side of spec.sides)add({family:"clock",side,exit:"fixed12h",timeframeMs:H,reference:"day",threshold:0,mode:"trend"});
  ...
  assert.equal(expected.size, 2808); assert.equal(summaries.length, expected.size);
  ...
  const referenceByTf=new Map<number,Map<number,any>>();
  const referenceKey=(r:any)=>r.timeframeMs;
  for(const tf of spec.timeframesMs){const values=referenceVwapVolume(candles,tf);referenceByTf.set(tf,values);
  ...
  for (const [key, r] of byCase) {
    ...
    const firstExit = (entered: number): Exit | null => {
      ...
      if (r.exit === "indicator_or12h") {
        ...
        // Only subsequent closed boundaries; do not reuse entry's VWAP/volume value.
    ...
    const validateEntry = (trade: any, label: string) => {
      ...
      if (r.family !== "clock") {
        ...
        assert.ok(crossed(r, references.get(signal - 2 * r.timeframeMs), references.get(signal - r.timeframeMs), references.get(signal - 3*r.timeframeMs)), `${label} independently reconstructed VWAP/volume crossing`);
      }
    ...
    for (const [i, t] of rows.entries()) {
      ...
      if (t.entryFeature !== null) {
        ...
        if(r.family!=="clock") {
          ...
          assert.equal(t.entryFeature.vvVersion,1);
        }
  ...
  for (const [key, r] of byCase) {
    ...
    if (r.exit === "fixed12h") assert.equal(r.deltaVsOwnFixed12h, null);
      ...
      reconstructed.get(caseKey({ ...r, id: fixedId(r) }))!.net, `${key} own fixed12h delta`);
    const unfiltered=plainId(r);assert.equal(r.unfilteredId,unfiltered);
    if(unfiltered)near(r.deltaVsUnfiltered,reconstructed.get(key)!.net-reconstructed.get(caseKey({...r,id:unfiltered}))!.net,`${key} own plain price delta`);
    else assert.equal(r.deltaVsUnfiltered,null);
    for (const [month, value] of reconstructed.get(key)!.markedByMonth) {
      const row = byMonth.get(`${key}|${month}`)!;
      assert.equal(row.unfilteredId,unfiltered);
      if(unfiltered){const plain=reconstructed.get(caseKey({...r,id:unfiltered}))!.markedByMonth.get(month)!;
        near(row.unfilteredMarkedNet,plain,`${key} plain monthly`);near(row.deltaVsUnfiltered,value-plain,`${key} plain monthly delta`);}
      else {assert.equal(row.unfilteredMarkedNet,null);assert.equal(row.deltaVsUnfiltered,null);}
  ...
  const withoutId=({id,unfilteredId,unfilteredMarkedNet,deltaVsUnfiltered,...rest}:any)=>{
    if(rest.entryFeature)rest.entryFeature=Object.fromEntries(Object.entries(rest.entryFeature).filter(([k])=>!k.startsWith("vv")));
    return rest;
  };
  let overlapCases=0;
  for(const r of summaries.filter(r=>r.family==="clock"||(r.family==="vwap"&&r.reference==="day"&&r.timeframeMs===H&&r.mode==="trend"&&r.threshold===0))){
    const oldId=r.family==="clock"?r.id:`vwap_cross_${r.side}_${r.exit}`;
  ...
  assert.equal(ranking.length, 700);
  const expectedRanking = strategyRules.map((rule: any) => {
    ...
    return { ...rule, passesResearchScreen: rule.family!=="bar_direction"&&!failures.length, failures, minNet: Math.min(...cs.map(r => r.net)),
  ...
  for(const [field,value]of Object.entries({missingMinutes:0,overlapParityCases:24,exactSavedOverlap:true,
    actualDataFeaturePrefixChecks:15,dayVwapAndRolling20SharedExact:true,actualDataEnginePrefixChecks:700,
    strategyDefinitions:640,priceControlDefinitions:60,totalDefinitionSlots:700,newDefinitions:696,repeatedDefinitions:4,
    controls:2,strategyCases:2560,priceControlCases:240,controlCases:8,inputAndSourceHashesUnchanged:true,liveChanges:0,fundingIncluded:false}))
    assert.deepEqual(validation[field],value,`Validation ${field}`);
  assert.equal(validation.zeroVolumeMinutes,candles.filter(c=>c.volume===0).length);
  assert.deepEqual(validation.passedResearchScreen,expectedRanking.filter(r=>r.passesResearchScreen).map(r=>r.id));
  assert.deepEqual(validation.deploymentCandidates,[]);
  const eligible = strategyRules.filter((rule:any)=>rule.family!=="bar_direction").filter((rule: any) => summaries.filter(r => r.id === rule.id)
  ...
  assert.deepEqual(shortlist.topFiveIds, (eligible.length ? eligible : expectedRanking.filter(r=>r.family!=="bar_direction")).slice(0,5).map((r:any)=>r.id));
  assert.deepEqual(shortlist.byFamily,["vwap","rvol"].map(family=>({family,
    eligibleIds:eligible.filter((r:any)=>r.family===family).map((r:any)=>r.id),
    topFiveIds:(eligible.some((r:any)=>r.family===family)?eligible:expectedRanking).filter((r:any)=>r.family===family).slice(0,5).map((r:any)=>r.id)})));
  ...
  const report = { version: 1, verifiedAt: new Date().toISOString(), passed: true,
    ...
    checks: ["independent actual-turnover day/week VWAP and rolling/same-time20 RVOL on five clocks; reset boundaries and direction controls",
      "every eligible flat signal and occupied skip independently enumerated", "terminal pending decisions independently derived",
      "full minute close/adverse equity drawdown independently reconstructed", "24 prior saved ledger/monthly/stat overlap cases",
    ...
    limitations: ["No strategy engine is imported; independent OHLC aggregation and anchored sums and prior-slot RVOL/crossing enumeration and ledger accounting check this fixed-timeout/neutral-exit study only.",
      "No untouched holdout or VWAP/volume combination tests; shortlist is descriptive, not live approval.",
