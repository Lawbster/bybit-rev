/** Independent C01 verifier: no combination/parent strategy engines imported.
...
import { referenceFeatures, referenceCross, referenceGate } from "./indicator-combination-reference";
...
async function main(){
  assert.equal(process.argv.length,3,"Pass the C01 artifact directory");const dir=path.resolve(root,process.argv[2]);
  ...
  assert.deepEqual(spec,read(path.join(root,"research-inputs/indicators/combinations-c01-2026-09-07.json")));
  assert.equal(spec.combinations.length,32);assert.equal(manifest.actualPrimaryCases,192);assert.equal(manifest.availabilityLogicalCases,128);
  ...
  assert.equal(results.length,320);assert.equal(diagnostics.length,128);const rows=new Map(results.map(r=>[key(r),r]));assert.equal(rows.size,320);
  ...
  const expectedIds=new Set([...manifest.rules.map((r:any)=>r.id),...spec.combinations.flatMap((c:any)=>[c.id,`ready_${c.id}`])]);assert.equal(expectedIds.size,80);
  ...
  for(const r of results){
    ...
    const rule=ruleMap.get(r.sourceParentId??r.id);assert(rule);for(const f of ["family","side","exit","timeframeMs","upper","period","threshold","mode","reference","fastPeriod","slowPeriod","signalPeriod"])assert.deepEqual(r[f],rule[f],`${k} rule ${f}`);
    ...
    for(const [i,t]of opportunities.entries()){
      const g=r.conditionId?referenceGate(r.conditionId,r.side,t,refs):null;
      ...
      if(r.family!=="clock"){
        ...
        if(g){assert(d.gate);assert.equal(d.gate.sourceStart,g.start);assert.equal(d.gate.sourceEnd,g.end);assert.equal(d.gate.expectedEnd,g.end);assert.equal(d.gate.availableAt,g.end);
          assert.equal(d.gate.ready,g.ready);assert.equal(d.gate.pass,g.pass);assert.equal(d.gate.dayStart,g.dayStart);
    ...
    reconstructed.set(k,{net,pm});if(reconstructed.size%32===0)console.log(`[verify] ${reconstructed.size}/320 cases; independent crossings/gates, ledgers and minute paths checked`);
  }
  ...
  const ranking=read(path.join(dir,"ranking.json")),shortlist=read(path.join(dir,"shortlist.json"));assert.equal(ranking.length,32);
  ...
  const parity=read(path.join(dir,"overlap-parity.json"));assert.equal(parity.length,64);assert(parity.every((p:any)=>p.exactSavedLedgerStatsMonths));
  const availability=read(path.join(dir,"availability-controls.json"));assert.equal(availability.length,128);
  ...
  fs.writeFileSync(destination,JSON.stringify({version:1,passed:true,verifiedAt:new Date().toISOString(),checkedCases:320,checkedTrades,checkedMonths,checkedDecisions,
  ...
  console.log(`[verified] ${checkedTrades} trade rows, ${checkedDecisions} crossing rows, ${checkedMonths} month rows; all320 cases passed`);
}
