/** Frozen VolumeFlow-only standalone research. New local artifacts; no live/config writes. */
import fs from "fs";
...
import { runVolumeFlowStandalone, volumeFlowEntrySignal, type VolumeFlowRule, type VolumeFlowResult } from "./volume-flow-standalone-engine";
import { computeVolumeFlowValues, VOLUME_FLOW_FORMULA_VERSION } from "../src/research/volume-flow-features";
...
async function main() {
  ...
  assert.equal(spec.holdMs, 12 * HOUR); assert.equal(spec.strategyDefinitionCount, 1800); assert.equal(spec.symbol, "HYPEUSDT");
  ...
  const sourceFiles = [definition, "scripts/volume-flow-standalone-engine.ts", "scripts/hype-volume-flow-standalone-study.ts",
    "src/research/volume-flow-features.ts", "scripts/volume-flow-standalone-tests.ts", "scripts/volume-flow-standalone-results-check.ts", "scripts/volume-flow-standalone-path-check.ts",
  ...
  const featureKey = (tf:number,family:string,n:number) => `${tf}:${family}:${n}`;
  const ruleKey = (r:VolumeFlowRule) => featureKey(r.timeframeMs,r.family,r.period);
  let prefixChecks = 0;
  for (const tf of spec.timeframesMs as number[]) {
    ...
    for (const p of spec.parameters) {
      const values=computeVolumeFlowValues(closed,tf,p.family,p.period);
      const rows=bars.bars.map((b,i)=>({...b,feature:{...base[i],...values[i],vfPeriod:p.period,vfFamily:p.family==="obv"?1:p.family==="mfi"?2:3}}));
      features.set(featureKey(tf,p.family,p.period),rows);
      for(const i of [800,Math.floor(rows.length/2),rows.length-3]){
        ...
        assert.deepEqual(computeVolumeFlowValues(prefix.bars.map(b=>b.candle),tf,p.family,p.period).at(-1),values[i]);
        assert(bars.windowAt(at,i+1).ready);prefixChecks++;
      }
    }
    console.log(`[features] ${tf/MINUTE}m ${oldRows.length} bars; nine parameter sets /27 prefixes; independent volume-flow formulas`);
  }
  const rules:VolumeFlowRule[]=[];
  for(const timeframeMs of spec.timeframesMs)for(const p of spec.parameters)
    for(const {mode,thresholds} of p.levels)for(const threshold of thresholds)for(const side of spec.sides)for(const exit of spec.exits)
      rules.push({id:`${p.family}_${timeframeMs/MINUTE}m_n${p.period}_${mode}_t${threshold}_${side}_${exit}`,
        family:p.family,timeframeMs,period:p.period,threshold,mode,side,exit});
  const controls:VolumeFlowRule[]=spec.sides.map((side:"long"|"short")=>({id:`clock_${side}`,family:"clock",side,
    exit:"fixed12h",timeframeMs:HOUR,period:14,threshold:0,mode:"trend"}));
  assert.equal(rules.length,1800);assert.equal(new Set(rules.map(r=>r.id)).size,1800);
  ...
  const cache = new Map<string, VolumeFlowResult>(), parity: any[] = [];
  for (const w of spec.windows) for (const delayMs of spec.executionDelaysMs) {
    for (const r of controls) {
      ...
      const fresh=runVolumeFlowStandalone(minutes,baseFeatures.get(HOUR)!,r,options(w,delayMs));
  ...
  json(path.join(out, "manifest.json"), { spec, node: process.version, sources, inputs, minuteRows: minutes.length, seed,
    formulaVersion: RESEARCH_FEATURE_FORMULA_VERSION + "|" + VOLUME_FLOW_FORMULA_VERSION, timingVersion: CLOSED_BAR_TIMING_VERSION,
  ...
  for (const w of spec.windows) for (const delayMs of spec.executionDelaysMs) for (const rule of [...controls, ...rules]) {
    ...
    const r = cache.get(key(w.id, delayMs, rule.id)) ?? runVolumeFlowStandalone(minutes, rows, rule, options(w, delayMs));
    ...
    if (rule.family !== "clock" && w.id === "full" && delayMs === 0 && r.trades.length) {
      ...
      assert(i>0); assert(volumeFlowEntrySignal(rule,rows[i-1],rows[i],rows[i-2]));
      const f=(j:number)=>{const x=rows[j].feature as any;return {vfRawObv:x.vfRawObv,vfPositive:x.vfPositive,vfNegative:x.vfNegative,vfVolume:x.vfVolume,vfNumerator:x.vfNumerator,vfDenominator:x.vfDenominator,vfValue:x.vfValue,vfSigned:x.vfSigned};};
  ...
  assert.equal(results.length, 7208);
  ...
  json(path.join(out, "shortlist.json"), { interpretation: "Descriptive positive/cost/sample subset; retain monthly/clock failures, no live approval.",
    ...
    byFamily:["obv","mfi","cmf"].map(family=>({family,eligibleIds:eligible.filter(x=>x.family===family).map(x=>x.id),
  ...
  for (const rule of rules) {
    ...
    assert.deepEqual(runVolumeFlowStandalone(minutes, rows, rule, opts), runVolumeFlowStandalone(
  ...
  json(path.join(out, "validation.json"), { missingMinutes: 0, overlapParityCases: parity.length, exactSavedOverlap: true,
    ...
    strategyDefinitions: rules.length, newDefinitions: 1800, repeatedDefinitions: 0, controls: 2, strategyCases: 7200, controlCases: 8,
