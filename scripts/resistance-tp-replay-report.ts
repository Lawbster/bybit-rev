/** Read-only rendering of accepted SRT03 evidence. */
...
const money=(n:number)=>Math.round(n).toLocaleString('en-US'),label=(x:R)=>x.policy==='baseline'?'B17':x.control?'Agg10 baseline':x.mode;
function table(headers:string[],data:any[][]){console.log('| '+headers.join(' | ')+' |\n|'+headers.map(()=>'---').join('|')+'|');for(const r of data)console.log('| '+r.join(' | ')+' |');}
for(const window of ['recent','published'])for(const tp of ['resting_touch','close_confirmed']){const rs=rows.filter(x=>x.window===window&&x.tp===tp&&!x.lag);
...
console.log('\nGeometry60 sensitivity\n');table(['Window','TP','Mode','Net','DD'],rows.filter(x=>x.lag).map(x=>[x.window,x.tp,x.mode,money(x.metrics.totalPnl),x.metrics.maxDrawdownPct.toFixed(2)]));
if(fs.existsSync(`${dir}/review.json`)){const r=read(`${dir}/review.json`);console.log(JSON.stringify({qualification:r.qualification,concentration:r.reviews.filter((x:R)=>!x.name.includes('lag60000')).map((x:R)=>({name:x.name,changedEpisodes:x.changedEpisodes,selectedEpisodes:x.selectedEpisodes,removed:x.removed,replacement:x.replacement,exLargest:x.netDeltaExcludingLargest,exTwo:x.netDeltaExcludingTwoLargest,largest:x.largestPositive[0]}))},null,2));}
