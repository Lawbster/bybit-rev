// SRL01 audit-only correction: reproduce the engine's boundary rebuild schedule.
/** SRL01 independent ledger, opportunity, predicate and unchanged-exit verification. */
...
async function main(){
 ...
 assert.equal(state.status,'complete');assert(!fs.existsSync(`${dir}/verification.json`));const pins=[...plan.pins,...plan.protectedPins,...state.artifacts];await verifyPins(process.cwd(),pins);const checkerBefore=sha(fs.readFileSync(__filename));
 ...
 for(const x of rows){console.log(`[SRL01 verify] ${x.name}`);
  ...
  const spec=POLICIES.find(p=>p.id===x.policy)!;assert.deepEqual(spec,x.spec);const c=config(cfg,spec);
  ...
  const sr=new ReplaySrContext(cs,c.srShadow!);let ptr=0,inv:R[]=[],cooldown=0,eligible=0,fireCount=0,prefix=0,lastBoundary=-Infinity;
  for(let i=x.startIdx;i<x.endIdx;i++){
   ...
   if(pct<.25)continue;
   const boundary=Math.floor(at/1800000)*1800000;
   if(boundary>lastBoundary){sr.at(boundary);lastBoundary=boundary;}
   const ct=sr.at(at);if(!ct.coverage.healthy)continue;
   const hit=ct.engine.nearestResistance(at,cs[i].close);
   const levelEligible=!!hit&&hit.dist<=.3/100&&hit.dist*100<=.3;
   ...
   const fire=deteriorating&&(x.mode==='without_resistance'||levelEligible),g=gateMap.get(i);assert(g,`Missing eligible minute ${x.name}/${i}`);
   ...
   assert.deepEqual(g.pulse,pulse);assert.equal(g.predicates.hostile,hostile);assert.equal(g.predicates.deteriorating,deteriorating);assert.equal(g.levelEligible,levelEligible);assert.equal(g.pulseChoice,deteriorating);
   if(hit){near(g.resistancePrice,hit.lv.price);near(g.resistanceDistPct,hit.dist*100);}else{assert.equal(g.resistancePrice,null);assert.equal(g.resistanceDistPct,null);}assert.equal(g.fire,fire);assert.equal(partMap.has(i),fire);
   ...
   if(prefix<2){const prefixContext=new ReplaySrContext(cs.filter(z=>z.endTs<=at),c.srShadow!);prefixContext.at(boundary);const pre=prefixContext.at(at);assert.deepEqual(pre.engine.getZones(at),ct.engine.getZones(at));prefix++;prefixChecks++;}
   if(traces.filter(t=>t.name===x.name).length<2||fire&&!levelEligible&&!traces.some(t=>t.name===x.name&&t.fire&&!t.levelEligible))traces.push({name:x.name,at:new Date(at).toISOString(),sourceAt:new Date(sourceAt).toISOString(),price:cs[i].close,depth:inv.length,
    resistance:hit?.lv.price??null,touches:hit?.lv.touchData??[],levelEligible,pct,planNet,pulse,hostile,deteriorating,fire,fill:partMap.get(i)?.event??null});gateChecks++;
  }
  assert.equal(eligible,gates.length);assert.equal(fireCount,partMap.size);
  if(x.control){const old=read(`${d.archive}/output/results.json`).find((r:R)=>r.name===x.name);assert.equal(old.digest,x.digest);assert.deepEqual(old.metrics,x.metrics);controls++;}
  else{const b=rows.find(r=>r.control&&r.policy===d.parent&&r.window===x.window&&r.tp===x.tp&&r.lag===x.lag)!,comparison=comparisons.find(r=>r.name===x.name)!;
   ...
   const changed=new Set(gates.filter(g=>g.fire&&!g.levelEligible).map(g=>g.episode)).size;
   assert.equal(changed,x.counts.changedEpisodes);
   reviews.push({name:x.name,changedEpisodes:changed,...cohort(x,b)});
  }
 }
 assert.equal(rows.length,16);assert.equal(controls,10);
 const qualification=d.variants.map((v:string)=>{const failures:string[]=[];
  ...
  for(const tp of d.tpModels){const b=rows.find(x=>x.mode==='current'&&x.window==='recent'&&x.tp===tp&&x.lag===60000)!,x=rows.find(x=>x.mode===v&&x.window==='recent'&&x.tp===tp&&x.lag===60000)!,q=exposureDelta(x.metrics,b.metrics);
 ...
 await verifyPins(process.cwd(),pins);assert.equal(sha(fs.readFileSync(__filename)),checkerBefore);atomicJson(`${dir}/review.json`,{qualification,reviews,traces});
 const receipt={passed:true,runs:16,exactControls:controls,fills,minutes,targetsChecked,highChecks,gateChecks,partialChecks,prefixChecks,
  reviewSha:sha(fs.readFileSync(`${dir}/review.json`)),checkerSha:checkerBefore,auditCorrection:'Original checker rebuilt S/R at a late eligible minute; corrected to closed 30m boundary rebuild then current-minute coverage/retention. Economic outputs and pinned original checker unchanged.',newTradingDefinitions:1,liveChanges:0};atomicJson(`${dir}/verification.json`,receipt);console.log(JSON.stringify(receipt));
}
