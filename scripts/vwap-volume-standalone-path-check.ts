/** Read-only I10 report supplement. No new signals, exits or strategy runs. */
...
async function main(): Promise<void> {
  ...
  const out = path.resolve(root, process.argv[2] ?? "backtests/hype/hype-vwap-volume-standalone-2026-09-06");
  ...
  const ids = ["clock_long", "clock_short", ...read("shortlist.json").topFiveIds, ...read("shortlist.json").byFamily.flatMap((r:any)=>r.topFiveIds), ...read("ranking.json").filter((r:any)=>r.family!=="bar_direction").slice(0,5).map((r:any)=>r.id)];
  const stats=read("results.json");
  for(const id of [...ids]){const r=stats.find((x:any)=>x.id===id);if(r?.unfilteredId)ids.push(r.unfilteredId);
    if(r?.exit==="indicator_or12h")ids.push(id.replace("indicator_or12h","fixed12h"));}
  const trades: any[] = [];
