...
// Independent direct minute aggregation and window formulas. No feature/strategy imports.
// Canonical summation order; fixtures separately test pairwise variance and the external library.
function referenceBollinger(candles:any[],tf:number,p:number[]):Map<number,any> {
  ...
  return new Map<number,any>(starts.map((ts,i):[number,any]=>{
    if(i<p[0]-1)return [ts,{bbMiddle:null,bbSd:null,bbUpper:null,bbLower:null,bbPercentB:null,bbBandwidthPct:null,bbCloseMinusMiddle:null}];
    const window=cs.slice(i-p[0]+1,i+1),middle=window.reduce((a,b)=>a+b,0)/p[0];
    const sd=Math.sqrt(window.reduce((a,b)=>a+(b-middle)**2,0)/p[0]),upper=middle+p[1]*sd,lower=middle-p[1]*sd,width=upper-lower;
    return [ts,{bbMiddle:middle,bbSd:sd,bbUpper:upper,bbLower:lower,bbPercentB:width===0?null:(cs[i]-lower)/width,bbBandwidthPct:100*width/middle,bbCloseMinusMiddle:cs[i]-middle}];
  }));
}
function crossed(r:any,a:any,b:any,_z?:any):boolean {
  const x=a?.bbPercentB,y=b?.bbPercentB;if(x==null||y==null)return false;
  if(r.mode==="breakout")return r.side==="long"?(x<=1&&y>1):(x>=0&&y<0);
  if(r.mode==="into")return r.side==="long"?(x>0&&y<=0):(x<1&&y>=1);
  return r.side==="long"?(x<=0&&y>0):(x>=1&&y<1);
}
function zeroExit(r:any,b:any):boolean {
  const v=b?.bbCloseMinusMiddle;if(v==null)return false;
  const signed=(r.side==="long"?1:-1)*v;return r.mode==="breakout"?signed<=0:signed>=0;
}

async function main(): Promise<void> {
  assert.equal(process.argv.length, 3, "Usage: npx ts-node scripts/bollinger-standalone-results-check.ts ARTIFACT_DIRECTORY");
  ...
  for(const timeframeMs of spec.timeframesMs)for(const period of spec.periods)for(const multiplier of spec.multipliers)
    for(const mode of spec.entryModes)for(const side of spec.sides)for(const exit of spec.exits)
      expectedRules.push({family:"bollinger",side,exit,timeframeMs,period,multiplier,mode,
        id:`bollinger_${timeframeMs/M}m_n${period}_k${multiplier}_${mode}_${side}_${exit}`});
  expectedRules.push(...spec.sides.map((side:string)=>({family:"clock",side,exit:"fixed12h",timeframeMs:H,
    period:20,multiplier:2,mode:"breakout",id:`clock_${side}`})));
  ...
  assert.equal(expected.size, 968); assert.equal(summaries.length, expected.size);
  ...
  const referenceKey=(r:any)=>`${r.timeframeMs}:${r.period}_${r.multiplier}`;
  for(const tf of spec.timeframesMs)for(const p of spec.periods.flatMap((n:number)=>spec.multipliers.map((k:number)=>[n,k]))){
    const values=referenceBollinger(candles,tf,p);referenceByTf.set(`${tf}:${p.join("_")}`,values);
  ...
  for (const [key, r] of byCase) {
    ...
    const firstExit = (entered: number): Exit | null => {
      ...
      if (r.exit === "indicator_or12h") {
        ...
        // Only subsequent closed boundaries; do not reuse entry's Bollinger value.
    ...
    const validateEntry = (trade: any, label: string) => {
      ...
      if (r.family !== "clock") {
        ...
        assert.ok(crossed(r, references.get(signal - 2 * r.timeframeMs), references.get(signal - r.timeframeMs), references.get(signal - 3*r.timeframeMs)), `${label} independently reconstructed Bollinger crossing`);
      }
    ...
    for (const [i, t] of rows.entries()) {
      ...
      if (t.entryFeature !== null) {
        ...
        if(r.family!=="clock") {
          ...
          assert.equal(t.entryFeature.bbPeriod,r.period);
          assert.equal(t.entryFeature.bbMultiplier,r.multiplier);
        }
  ...
  assert.equal(ranking.length, 240);
  ...
  const report = { version: 1, verifiedAt: new Date().toISOString(), passed: true,
    ...
    checks: ["independent Bollinger formulas for all five timeframes / four parameter pairs; breakout/fade/reclaim crossings",
    ...
    limitations: ["No strategy engine is imported; independent direct-window Bollinger formulas/crossing enumeration and ledger accounting check this fixed-timeout/neutral-exit study only.",
      "No untouched holdout or Bollinger combination tests; shortlist is descriptive, not live approval.",
