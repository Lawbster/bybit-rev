...
// Independently aggregate each complete minute block and index its N-bar
// denominator. No production indicator, closed-bar, or strategy engine import.
function referenceRoc(candles: any[], tf: number, period: number): Map<number, number | null> {
  const step = tf / M, values = new Map<number, number | null>();
  for (let i = 0; i + step <= candles.length; i += step) {
    const numerator = candles[i + step - 1].close;
    const value = i < period * step ? null : 100 * (numerator / candles[i - period * step + step - 1].close - 1);
    values.set(candles[i].ts, value);
  }
  return values;
}
function crossed(r: any, previous: number | null | undefined, current: number | null | undefined): boolean {
  if (previous == null || current == null) return false;
  const signedPrevious = (r.side === "long" ? 1 : -1) * previous;
  const signedCurrent = (r.side === "long" ? 1 : -1) * current;
  if (r.mode === "momentum") return signedPrevious <= r.magnitudePct && signedCurrent > r.magnitudePct;
  if (r.mode === "into") return signedPrevious > -r.magnitudePct && signedCurrent <= -r.magnitudePct;
  return signedPrevious <= -r.magnitudePct && signedCurrent > -r.magnitudePct;
}
function zeroExit(r: any, value: number | null | undefined): boolean {
  if (value == null) return false;
  const signed = (r.side === "long" ? 1 : -1) * value;
  return r.mode === "momentum" ? signed <= 0 : signed >= 0;
}

async function main(): Promise<void> {
  assert.equal(process.argv.length, 3, "Usage: npx ts-node scripts/roc-standalone-results-check.ts ARTIFACT_DIRECTORY");
  ...
  for (const timeframeMs of spec.timeframesMs) for (const lookbackBars of spec.lookbackBars)
    for (const magnitudePct of [0, ...spec.magnitudesPct]) for (const mode of magnitudePct === 0 ? ["momentum"] : spec.entryModes)
      for (const side of spec.sides) for (const exit of spec.exits) expectedRules.push({
        family: "roc", side, exit, timeframeMs, lookbackBars, magnitudePct, mode,
        id: `roc_${timeframeMs / M}m_n${lookbackBars}_m${magnitudePct}_${mode}_${side}_${exit}` });
  expectedRules.push(...spec.sides.map((side: string) => ({ family: "clock", side, exit: "fixed12h",
    timeframeMs: H, lookbackBars: 5, magnitudePct: 0, mode: "momentum", id: `clock_${side}` })));
  ...
  assert.equal(expected.size, 2408); assert.equal(summaries.length, expected.size);
  ...
  const referenceByTf = new Map<string, Map<number, number | null>>();
  const referenceKey = (r: any) => `${r.timeframeMs}:${r.lookbackBars}`;
  for (const tf of spec.timeframesMs) for (const period of spec.lookbackBars) {
    const values = referenceRoc(candles, tf, period); referenceByTf.set(`${tf}:${period}`, values);
    const saved = manifest.closedBars.find((b: any) => b.timeframeMs === tf); assert.ok(saved);
    assert.equal(saved.count, values.size, `Complete ${tf / M}m bar count`);
  }
  ...
  for (const [key, r] of byCase) {
    ...
    const firstExit = (entered: number): Exit | null => {
      ...
      if (r.exit === "indicator_or12h") {
        ...
        // Only subsequent closed boundaries; do not reuse entry's ROC value.
    ...
    const validateEntry = (trade: any, label: string) => {
      ...
      if (r.family !== "clock") {
        ...
        assert.ok(crossed(r, references.get(signal - 2 * r.timeframeMs), references.get(signal - r.timeframeMs)), `${label} independently reconstructed ROC crossing`);
      }
    ...
    for (const [i, t] of rows.entries()) {
      ...
      if (t.entryFeature !== null) {
        ...
        const expectedRoc = referenceByTf.get(referenceKey(r))!.get(featureStart);
        const field = r.family === "clock" ? "roc5" : "roc";
        if (r.family !== "clock") assert.equal(t.entryFeature.rocLookbackBars, r.lookbackBars);
        if (expectedRoc == null) assert.equal(t.entryFeature[field], null);
        else near(t.entryFeature[field], expectedRoc, `${label} independently calculated ROC`, 1e-8);
    ...
    if (r.family === "clock") {
      ...
      for (const [barStart, current] of reference) {
        ...
        if (at >= start && at < stop && crossed(r, reference.get(barStart - r.timeframeMs), current)) opportunities.add(at);
      }
    }
  ...
  // Four repeated ROC definitions and the two shared clocks must retain their
  // old complete trade/monthly paths. New rule metadata is intentionally different.
  ...
  const withoutId = ({ id, ...rest }: any) => {
    if (!rest.entryFeature || !("roc" in rest.entryFeature)) return rest;
    const { roc, rocLookbackBars, ...entryFeature } = rest.entryFeature;
    assert.equal(rocLookbackBars, 5); assert.equal(roc, entryFeature.roc5);
    return { ...rest, entryFeature };
  };
  let overlapCases = 0;
  for (const r of summaries.filter(r => r.family === "clock" || r.timeframeMs === H && r.lookbackBars === 5 && r.magnitudePct === 0)) {
    const oldId = r.family === "clock" ? r.id : `roc_cross_${r.side}_${r.exit}`;
    ...
    assert.deepEqual(monthly.filter(m => caseKey(m) === caseKey(r)).map(withoutId), priorMonths.filter(matches).map(withoutId), "Prior verified monthly overlap");
    overlapCases++;
  }
  ...
  assert.equal(ranking.length, 600);
  ...
  for (const trace of traces) {
    ...
    const ref = referenceByTf.get(referenceKey(r))!;
    near(trace.previousRoc, ref.get(first.signalAt - 2 * r.timeframeMs)!, "trace previous ROC");
    near(trace.currentRoc, ref.get(first.signalAt - r.timeframeMs)!, "trace current ROC");
    assert.equal(trace.previousBarStart, first.signalAt - 2 * r.timeframeMs);
    assert.equal(trace.currentBarStart, first.signalAt - r.timeframeMs);
    assert.equal(trace.availableAt, first.signalAt);
    const denom = first.signalAt - (r.lookbackBars + 1) * r.timeframeMs;
    assert.equal(trace.denominatorBarStart, denom);
    assert.equal(trace.previousDenominatorBarStart, denom - r.timeframeMs);
    near(trace.denominatorClose, candle(denom + r.timeframeMs - M).close, "trace historical denominator price", 1e-10);
    near(trace.numeratorClose, candle(first.signalAt - M).close, "trace closed numerator price", 1e-10);
  ...
  const report = { version: 1, verifiedAt: new Date().toISOString(), passed: true,
    ...
    checks: ["independent ROC formulas for all five timeframes / three lookbacks and momentum/into/back-out crossings",
    ...
    limitations: ["No strategy engine is imported; independent scalar ROC formulas/crossing enumeration and ledger accounting check this fixed-timeout/neutral-exit study only.",
      "No untouched holdout or ROC combination tests; shortlist is descriptive, not live approval.",
