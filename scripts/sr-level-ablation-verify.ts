...
async function main(){
 ...
 for(const x of rows){console.log(`[SRL01 verify] ${x.name}`);
  ...
  for(let i=x.startIdx;i<x.endIdx;i++){
   ...
   if(pct<.25)continue;
   const boundary=Math.floor(at/1800000)*1800000,ct=sr.at(at);if(!ct.coverage.healthy)continue;
   ...
   if(prefix<2){const pre=new ReplaySrContext(cs.filter(z=>z.endTs<=at),c.srShadow!).at(at);assert.deepEqual(pre.engine.getZones(at),ct.engine.getZones(at));prefix++;prefixChecks++;}
 ...
 const receipt={passed:true,runs:16,exactControls:controls,fills,minutes,targetsChecked,highChecks,gateChecks,partialChecks,prefixChecks,
  reviewSha:sha(fs.readFileSync(`${dir}/review.json`)),checkerSha:sha(fs.readFileSync(__filename)),newTradingDefinitions:1,liveChanges:0};atomicJson(`${dir}/verification.json`,receipt);console.log(JSON.stringify(receipt));
}
