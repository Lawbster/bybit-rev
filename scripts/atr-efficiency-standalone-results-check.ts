...
// Independent OHLC aggregation, array-form ATR and direct slice ER; no feature/strategy imports.
function referenceAtrEfficiency(candles:any[],tf:number,family:string,n:number):Map<number,any>{
  const step=tf/M,bars:any[]=[];
  for(let i=0;i+step<=candles.length;i+=step){
    let high=-Infinity,low=Infinity;
    for(let j=i;j<i+step;j++){high=Math.max(high,candles[j].high);low=Math.min(low,candles[j].low);}
    bars.push({ts:candles[i].ts,high,low,close:candles[i+step-1].close});
  }
  const changes=bars.map((b,i)=>i?b.close-bars[i-1].close:null);
  const atr:Array<number|null>=Array(bars.length).fill(null);
  if(family==="atr_move"){
    const tr=bars.map((b,i)=>i?Math.max(b.high-b.low,Math.abs(b.high-bars[i-1].close),Math.abs(b.low-bars[i-1].close)):null);
    if(bars.length>14)atr[14]=tr.slice(1,15).reduce((a,b)=>a!+b!,0)!/14;
    for(let i=15;i<bars.length;i++)atr[i]=(atr[i-1]!*13+tr[i]!)/14;
  }
  return new Map(bars.map((b,i):[number,any]=>{
    const x:any={aeAtr:null,aePriorAtr:null,aeChange:null,aeTravel:null,aeUnsigned:null,aeSigned:null};
    if(family==="atr_move"&&i){
      x.aeAtr=atr[i];x.aePriorAtr=atr[i-1];x.aeChange=changes[i];
      if(atr[i-1]!=null&&atr[i-1]!>0){x.aeSigned=changes[i]!/atr[i-1]!;x.aeUnsigned=Math.abs(x.aeSigned);}
    }else if(family==="efficiency"&&i>=n){
      x.aeChange=b.close-bars[i-n].close;x.aeTravel=changes.slice(i-n+1,i+1).reduce((a,v)=>a!+Math.abs(v!),0);
      x.aeSigned=x.aeTravel===0?0:x.aeChange/x.aeTravel;x.aeUnsigned=Math.abs(x.aeSigned);
    }
    return[b.ts,x];
  }));
}
function crossed(r:any,a:any,b:any,_z?:any):boolean{
  if(a?.aeSigned==null||b?.aeSigned==null)return false;
  const sign=r.side==="long"?1:-1,p=a.aeSigned*sign,c=b.aeSigned*sign;
...
function zeroExit(r:any,b:any):boolean{
  if(b?.aeSigned==null)return false;const v=(r.side==="long"?1:-1)*b.aeSigned;
  return r.mode==="trend"?v<=0:v>=0;
}

async function main(): Promise<void> {
  assert.equal(process.argv.length, 3, "Usage: npx ts-node scripts/atr-efficiency-standalone-results-check.ts ARTIFACT_DIRECTORY");
  ...
  const definition = "research-inputs/indicators/atr-efficiency-standalone-2026-09-06.json";
  ...
  assert.deepEqual(spec.parameters,[{family:"atr_move",period:14,thresholds:[.5,1,2]},...[10,20,40].map(period=>({family:"efficiency",period,thresholds:[.25,.5,.75]}))]);
  ...
  for(const timeframeMs of spec.timeframesMs)for(const p of spec.parameters)
    for(const threshold of p.thresholds)for(const mode of spec.entryModes)for(const side of spec.sides)for(const exit of spec.exits)
  ...
  assert.equal(expected.size, 2888); assert.equal(summaries.length, expected.size);
  ...
  for(const tf of spec.timeframesMs)for(const p of spec.parameters){
    const values=referenceAtrEfficiency(candles,tf,p.family,p.period);referenceByTf.set(`${tf}:${p.family}:${p.period}`,values);
  ...
  for (const [key, r] of byCase) {
    ...
    const firstExit = (entered: number): Exit | null => {
      ...
      if (r.exit === "indicator_or12h") {
        ...
        // Only subsequent closed boundaries; do not reuse entry's ATR/efficiency value.
    ...
    const validateEntry = (trade: any, label: string) => {
      ...
      if (r.family !== "clock") {
        ...
        assert.ok(crossed(r, references.get(signal - 2 * r.timeframeMs), references.get(signal - r.timeframeMs), references.get(signal - 3*r.timeframeMs)), `${label} independently reconstructed ATR/efficiency crossing`);
      }
    ...
    for (const [i, t] of rows.entries()) {
      ...
      if (t.entryFeature !== null) {
        ...
        if(r.family!=="clock") {
          ...
          assert.equal(t.entryFeature.aePeriod,r.period);assert.equal(t.entryFeature.aeFamily,r.family==="atr_move"?1:2);
        }
  ...
  assert.equal(ranking.length, 720);
  ...
  assert.deepEqual(shortlist.byFamily,["atr_move","efficiency"].map(family=>({family,
  ...
  const report = { version: 1, verifiedAt: new Date().toISOString(), passed: true,
    ...
    checks: ["independent ATR/efficiency formulas for all five timeframes; ATR14 prior scale and signed ER10/20/40; trend/into/recovery crossings",
    ...
    limitations: ["No strategy engine is imported; independent OHLC aggregation and array-form ATR/efficiency recurrences/crossing enumeration and ledger accounting check this fixed-timeout/neutral-exit study only.",
      "No untouched holdout or ATR/efficiency combination tests; shortlist is descriptive, not live approval.",
