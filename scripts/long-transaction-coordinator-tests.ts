...
function fakeExecutor(options: FakeOptions): Executor {
  ...
  return {
    ...
    getLongPositionProtection: async () => ({ size: 0, takeProfit: 0, stopLoss: 0, positionIdx: 1, updatedTime: 0 }),
    openLongDetailed: async () => options.openResult ?? result({}),
    closeAllLongsDetailed: async () => options.closeResult ?? result({}),
    placeLongMakerTpDetailed: async () => result({}),
    cancelLongMakerTpDetailed: async () => result({ outcome: "terminal", status: "Cancelled", terminal: true }),
    ...
    clearPositionTp: async () => ({ success: true, status: "confirmed" }),
