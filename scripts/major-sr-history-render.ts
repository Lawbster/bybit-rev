...
export function levelTable(m:MajorMap){
  const title=`# Historical major S/R levels — ${m.symbol}, 4H\n\n`;
  const note=`Full canonical source: **${iso(m.sourceStart)} to ${iso(m.sourceEnd)}**. Last complete 4H bar closes ${iso(m.bars.at(-1)!.endTs)}.\n\n`+
...
export function renderMap(m:MajorMap):string{
  const payload=JSON.stringify({symbol:m.symbol,cutoff:m.cutoff,sourceStart:m.sourceStart,levels:m.levels,
    frames:m.frames,bars:m.bars,intervals:m.intervals}).replace(/</g,'\\u003c');
  return `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width">
<title>${m.symbol} — historical 4H S/R map</title><style>
...
</style><h1>${m.symbol} — full-history major S/R, 4H</h1>
...
<canvas id="chart"></canvas><p class="note">White line: closed 4H price. Selected zone: shaded band. No final-map lines are projected backward. Early/incomplete coverage is marked not ready even where qualified levels are visible.</p><div id="detail"></div>
...
function render(){const f=frame(),active=new Map((f?.levels||[]).map(z=>[z.id,z])),expected=Math.floor(at/14400000)*14400000;
